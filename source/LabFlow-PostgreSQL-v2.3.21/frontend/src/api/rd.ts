...
export const getRdSampleWorkloadPreview = (id: number): Promise<ApiResponse<SampleWorkloadPreview>> =>
  client.get(`/rd-records/${id}/sample-workload`).then(r => r.data);

export const createRdSampleWorkload = (id: number, data: { quantity: number; multiplier: number; notes?: string }): Promise<ApiResponse<WorkRecord>> =>
  client.post(`/rd-records/${id}/sample-workload`, data).then(r => r.data);
