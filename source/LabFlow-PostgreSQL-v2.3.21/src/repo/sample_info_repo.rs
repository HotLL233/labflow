...
pub fn list(pool: &DbPool, q: &SampleInfoQuery) -> Result<(Vec<SampleInfoResponse>, i64)> {
    ...
    let sql = format!(
        "SELECT id, business_no, status, seq_no, batch_no, user_name, lab_name, project_name, \
         ...
         COALESCE((SELECT SUM(wr.quantity) FROM work_records wr WHERE wr.source_type IN ('sample_info_sample','sample_info') AND wr.source_record_id=sample_info_records.id AND wr.deleted_at IS NULL),0) >= sample_info_records.quantity \
