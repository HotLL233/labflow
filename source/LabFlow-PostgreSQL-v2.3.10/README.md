...
`v2.3.10` 是基于 PostgreSQL 的样品送样、分析检测工作量与样品信息管理系统版本，完善工作量录入权限白名单和已有角色权限迁移。
...
- 人员变动通知与关联项目多选。
- 样品信息登记取样后支持按当前操作账号录入工作量；多个仪器时必须选择实际仪器。
- 样品登记记录的“编辑”“录入工作量”“退回”按钮支持在管理后台启用、停用、排序和自定义宽度。
- 录入工作量权限会显示在角色权限管理中，已有具备取样权限的角色会自动补齐对应权限。

## Windows 安装

从发布目录下载 `LabFlow-v2.3.10.exe`。
...
sudo dpkg -i ../releases/labflow_2.3.10_amd64.deb
