...
const SampleInfoEntry: React.FC = () => {
  ...
  // 桌面端列宽直接使用后台配置的像素值。之前先按内容计算再归一化为百分比，
  // 让所有列总宽始终等于视口宽度，导致列宽配置看起来不生效且表头被挤成竖排。
  const recordDisplayWidths = useMemo(() => {
    ...
    displayColumns.forEach(column => {
      ...
      widths[column.field_key] = { desktop: `${width}px`, mobile: width };
    });
    ...
    const actionWidth = Math.max(280, Math.min(520, configuredActionWidth + 24));
    widths._action = { desktop: `${actionWidth}px`, mobile: actionWidth };
    return widths;
