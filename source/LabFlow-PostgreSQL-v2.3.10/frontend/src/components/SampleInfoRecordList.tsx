...
export default function SampleInfoRecordList(props: SampleInfoRecordListProps) {
  ...
  const AllActions = ({ record }: { record: SampleInfoRecord }) => (
    <Box sx={{ display: 'grid', gap: 0.5, width: '100%', minWidth: 0 }}>
      <ConfiguredActions record={record} />
      <RecordActions record={record} />
    </Box>
  );

  const ColumnValue = ({ record, column }: { record: SampleInfoRecord; column: SampleInfoColumn }) => {
    if (column.field_key === 'status') return <Box sx={valueBoxSx}><Box sx={{ display: 'grid', gap: 0.35 }}><StatusChip status={record.status} />{record.return_reason && <Wrap muted>原因：{record.return_reason}</Wrap>}</Box></Box>;
  ...
  const parseDesktopWidth = (value: string | undefined, fallback: number) => {
    const parsed = Number.parseFloat(value || '');
    return Number.isFinite(parsed) && parsed > 0 ? parsed : fallback;
  };
  const tableWidth = dataColumns.reduce((sum, column) => sum + parseDesktopWidth(columnWidths[column.field_key]?.desktop, 120), 0)
    + parseDesktopWidth(columnWidths._action?.desktop, 320);
      ...
      {loading ? <Box sx={{ textAlign: 'center', py: 4 }}><CircularProgress size={32} /></Box> : (
        <>
          <TableContainer className={extraClass} sx={{ display: { xs: 'none', lg: 'block' }, width: '100%', overflowX: 'auto', border: '1px solid #e5e5e5', borderRadius: R }}>
            <Table size="small" sx={{ width: `${tableWidth}px`, minWidth: '100%', tableLayout: 'fixed' }}>
              <colgroup>
                ...
                {records.length === 0 ? <TableRow><TableCell colSpan={Math.max(2, dataColumns.length + 1)} align="center" sx={{ py: 4, color: 'text.secondary' }}>暂无登记记录</TableCell></TableRow> : records.map(record => (
                  <React.Fragment key={record.id}>
                    <TableRow hover sx={{ bgcolor: record.status === STATUS_DONE ? '#fbfbfb' : '#fff', verticalAlign: 'top', '& td': { px: 0.8, py: 1 } }}>
                      {dataColumns.map(column => <TableCell key={column.field_key} sx={{ ...recordCellSx, width: columnWidths[column.field_key]?.desktop }}><ColumnValue record={record} column={column} /></TableCell>)}
                      <TableCell sx={{ ...recordCellSx, width: columnWidths._action?.desktop, px: 0.5, textAlign: 'center' }}><AllActions record={record} /></TableCell>
                    </TableRow>
              ...
              <Card key={record.id} variant="outlined" sx={{ borderRadius: R, borderLeft: `4px solid ${record.status === STATUS_WAIT_SAMPLE ? '#d32f2f' : record.status === STATUS_WAIT_TEST ? '#f9a825' : '#2e7d32'}` }}>
                <CardContent sx={{ p: 1.25, '&:last-child': { pb: 1.25 } }}>
                  ...
                  {editable(record) ? <Box onClick={event => event.stopPropagation()} onMouseDown={event => event.stopPropagation()} sx={{ display: 'grid', gap: 1 }}>
                    ...
                    <Box sx={{ mt: { xs: 1.75, lg: 1 }, display: 'grid', gap: 0.75 }}><AllActions record={record} /></Box>
                  </>}
                </CardContent>
              </Card>
            ))}
