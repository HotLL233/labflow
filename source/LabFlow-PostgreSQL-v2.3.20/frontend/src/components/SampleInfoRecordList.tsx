...
export default function SampleInfoRecordList(props: SampleInfoRecordListProps) {
  const {
    ...
    actionCollapsed = false, widthOverrides, containerRef,
  } = props;
      ...
      {loading ? <Box sx={{ textAlign: 'center', py: 4 }}><CircularProgress size={32} /></Box> : (
        <>
          <TableContainer ref={containerRef} className={extraClass} sx={{ display: { xs: 'none', lg: 'block' }, width: '100%', maxHeight: '72vh', overflow: 'auto', border: '1px solid #e5e5e5', borderRadius: R }}>
            <Table size="small" stickyHeader sx={{ width: '100%', minWidth: 0, tableLayout: 'fixed', ...stickyHeaderSx }}>
              <colgroup>
                {visibleColumns.map(column => <col key={column.field_key} style={{ width: columnWidths[column.field_key]?.desktop }} />)}
                <col style={{ width: columnWidths._action?.desktop }} />
              </colgroup>
              <TableHead><TableRow>
                {visibleColumns.map(column => {
                  const sortable = SORTABLE_FIELDS.has(column.field_key);
                  return <TableCell key={column.field_key} sx={{ position: 'relative', fontWeight: 700, whiteSpace: 'normal', overflowWrap: 'break-word', wordBreak: 'normal', px: 0.8, py: 1, borderColor: '#e0e0e0' }}>
                    {widthOverrides && (
                      <ColumnResizeHandle
              ...
              <TableBody>
                {filteredRecords.length === 0 ? (
                  <TableRow><TableCell colSpan={Math.max(2, visibleColumns.length + 1)} align="center" sx={{ py: 4, color: 'text.secondary' }}>
                  ...
                  <React.Fragment key={record.id}>
                    ...
                    {editable(record) && <SampleInfoEditRow record={record} columnCount={visibleColumns.length + 1} editColumns={editColumns} editForm={editForm} formatDate={formatDate} onEditFormChange={onEditFormChange} onCancel={onCancelEdit} onSave={onSaveEdit} divisionOptions={divisionOptions} />}
                  </React.Fragment>
                ))}
              </TableBody>
            </Table>
