...
pub fn exists_source(pool: &DbPool, source_type: &str, source_record_id: i64) -> Result<bool> {
    let conn = pool.get()?;
    Ok(conn.query_row(
        // The source pair is protected by a unique index regardless of the
        // recycle-bin state. Treat a soft-deleted source record as already
        // recorded so a second sampling operation cannot leak a PostgreSQL
        // unique-constraint error to the client.
        "SELECT EXISTS(SELECT 1 FROM work_records WHERE source_type=?1 AND source_record_id=?2)",
...
pub fn exists_sample_info_source(pool: &DbPool, source_record_id: i64) -> Result<bool> {
    let conn = pool.get()?;
    Ok(conn.query_row(
        "SELECT EXISTS(SELECT 1 FROM work_records WHERE source_type IN ('sample_info_sample','sample_info') AND source_record_id=?1)",
        [source_record_id],
        |row| row.get(0),
    )?)
}
...
pub fn create(pool: &DbPool, body: &RecordCreate, operator: &str) -> Result<RecordResponse> {
    create_inner(pool, body, operator, true)
}
...
pub fn create_sample_workload(
    ...
    create_inner(pool, body, operator, false)
}
...
fn create_inner(
    ...
    let inserted = match tx.execute(
        ...
        postgres_compat::params![body.project_id,body.method_id,&body.user_name,body.quantity,&body.recorded_at,
            ...
            body.source_type,body.source_record_id],
    ) {
        Ok(inserted) => inserted,
        Err(postgres_compat::Error::Database(message))
            if message.contains("uq_work_records_sample_source") =>
        {
