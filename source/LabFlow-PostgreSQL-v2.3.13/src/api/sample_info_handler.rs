...
fn workload_instruments(
    ...
    // 优先级：项目已关联的方法优先；项目未配置方法关联时，回退到按检测类型匹配
    // 的通用方法。方法未绑定仪器（或仪器已停用）时仍返回候选，仪器名称显示为方法名，
    // 录入的工作量按方法本身计算，避免历史配置无法录入。
...
fn workload_unavailable_reason(record: &SampleInfoResponse) -> Option<String> {
    if record.project_name.trim().is_empty() {
        return Some("该样品记录没有填写项目名称，无法匹配检测方法。".into());
    }
    Some(format!(
        "项目「{}」下没有可用的检测方法。请检查：项目是否已关联方法、方法是否启用并在样品信息登记中显示、方法的检测类型是否为「{}」。",
...
async fn sample_workload_preview(
    ...
    let reason = if instruments.is_empty() {
