...
import { ColumnResizeHandle, RowActionsMenu, useColumnWidthOverrides, useContainerWidth } from '../components/recordTable/columnLayoutHooks';
...
const RdRecordsPage: React.FC = () => {
  ...
  const columnLayouts = useMemo(() => {
    const inputs = [
      ...
      ...visibleColumns.map(col => {
        ...
        const custom = hasOverride || col.width_mode === 'custom';
        return {
          key: col.name,
          header: col.label,
  ...
  const physicalColumnKeys = useMemo(
    () => ['_select', ...visibleColumns.map(col => col.name), '_action'],
    [visibleColumns],
  );
  // 使用实际像素宽度，避免 custom 列先转百分比后又被 tableLayout: fixed 二次压缩。
  const tableContentWidth = physicalColumnKeys.reduce((sum, key) => sum + (columnLayouts[key]?.px || 0), 0);
      ...
      {records.length === 0 ? (
        ...
        <TableContainer ref={tableBoxRef} component={Paper} variant="outlined" sx={{ borderRadius: R, boxShadow: 'none', maxHeight: '72vh', overflow: 'auto' }}>
          <Table size="small" stickyHeader sx={{
            ...adaptiveTableSx,
            ...stickyHeaderSx,
            width: `max(100%, ${tableContentWidth}px)`,
            ...
            <TableHead>
              <TableRow>
                ...
                <TableCell sx={{ fontWeight: 700, fontSize: '0.78rem', ...cellWidthSx('_action'), px: 0.5, py: 1, textAlign: 'center' }}>操作</TableCell>
              </TableRow>
            </TableHead>
            <TableBody>
              ...
              {filteredRecords.map((rec, idx) => {
                ...
                return (
                  <TableRow key={rec.id} hover selected={selectedIds.includes(rec.id)} sx={{ '&:last-child td': { borderBottom: 0 }, verticalAlign: 'top', '& > td': { minHeight: `${tableConfig.row_height}px` } }}>
                    ...
                    <TableCell sx={[recordCellSx, cellWidthSx('_action'), { px: 0.5, textAlign: 'center', whiteSpace: 'nowrap', height: tableConfig.row_height }]}>
                      {isEditing ? (
                        ...
                        <RowActionsMenu
                          collapsed={actionLayout.collapsed}
                          maxInline={actionLayout.tier === 'text' ? 5 : 1}
                          items={[
