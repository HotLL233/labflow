...
import { computeActionColumnLayout, computeRecordColumnWidths } from '../utils/recordTableLayout';
import { useColumnWidthOverrides, useContainerWidth } from '../components/recordTable/columnLayoutHooks';
import { useActionColumnMode } from '../components/recordTable/RecordTableTools';
...
const ACTION_FIELDS = new Set(['action_edit', 'action_record_workload', 'action_return', 'action_collect', 'action_withdraw', 'action_complete']);
...
const SampleInfoEntry: React.FC = () => {
  ...
  // v2.3.13: 管理后台调整列配置（宽度、显示开关）后，切回本页即可生效，无需刷新浏览器。
  useEffect(() => {
    const reloadColumns = () => {
      getActiveSampleInfoColumns(recordTypeFilter || undefined).then(response => {
        if (response.code === 0 && response.data) setRecordColumns(response.data);
    ...
    window.addEventListener('focus', reloadColumns);
    return () => window.removeEventListener('focus', reloadColumns);
  }, [recordTypeFilter]);

  // v2.3.13: 返回本次加载的记录，便于录入工作量后核对服务端状态。
  const load = useCallback(async (): Promise<SampleInfoRecord[]> => {
    setLd(true);
    try {
  ...
  const legacyListColumnCount = listColumns.length + 2;

  const listRecordWidths = useMemo(() => getAdaptiveColumnWidths(records, [
    { key: 'status', header: '状态', fixed: 76, getValue: r => r.status },
    { key: 'seq_no', header: '序号', fixed: tableConfig.seq_column_width || 54, getValue: r => `${r.seq_no} ${r.business_no || ''}` },
  ...
  // 桌面端将后台配置的像素宽度转换为比例，保证整张表始终在页面内显示。
  // 像素值仍作为比例权重保留，单元格内部负责长内容的滚动。
  // v2.3.20：容器宽度测量、本机列宽覆盖、操作列收纳档位。
  const { ref: recordTableRef, width: recordTableWidth } = useContainerWidth<HTMLDivElement>();
  const recordWidthOverrides = useColumnWidthOverrides(user?.id ? `sample-info:${user.id}` : 'sample-info');
  const actionColumnMode = useActionColumnMode();

  /** v2.3.20：操作列宽度按容器宽度取档；关闭收纳开关时回到按按钮宽度累加。 */
  const recordActionWidth = useMemo(() => {
    const buttonWidthSum = recordColumns
      .filter(column => column.show_in_list && column.data_type === 'action')
      .reduce((sum, column) => sum + Math.max(48, Math.min(Number(column.width) || 96, 500)), 0);
    return computeActionColumnLayout(actionColumnMode, recordTableWidth, buttonWidthSum).px;
  }, [recordColumns, actionColumnMode, recordTableWidth]);

  const recordDisplayWidths = useMemo(() => {
    const displayColumns = recordColumns.filter(column => column.show_in_list && column.data_type !== 'action');
    const actionColumns = recordColumns.filter(column => column.show_in_list && column.data_type === 'action');
    const inputs = [
      ...displayColumns.map(column => {
        const override = recordWidthOverrides.overrides[column.field_key];
        const hasOverride = override !== undefined;
        const custom = hasOverride || column.width_mode === 'custom';
        const isSeq = column.field_key === 'seq_no';
        return {
          key: column.field_key,
          header: column.label,
          dataType: column.data_type,
          width: override ?? column.width,
          widthMode: custom ? ('custom' as const) : ('auto' as const),
          // v2.3.20：序号列宽接通后台「表格配置」，作为下限参与内容测量。
          minWidth: isSeq ? Math.max(tableConfig.seq_column_width || 54, column.min_width || 0) : column.min_width,
          maxWidth: column.max_width,
          // 长内容列优先吸收富余，让批号 / 主要成分 / 注意事项尽量完整显示。
          extendable: ['batch_no', 'main_components', 'notes'].includes(column.field_key),
          getValue: (record: SampleInfoRecord) => getRecordValue(record, column.field_key),
        };
      }),
      {
        key: '_action',
        header: '操作',
        dataType: 'action',
        fixed: recordActionWidth,
        getValue: () => '',
      },
    ];
    const layouts = computeRecordColumnWidths(records, inputs, { containerWidth: recordTableWidth });
    // 记录列表沿用 desktop（百分比）/ mobile（像素）两个口径，避免改动其内部取宽逻辑。
    return Object.fromEntries(
      Object.entries(layouts).map(([key, item]) => [key, { desktop: `${item.px}px`, mobile: item.px }]),
    );
  }, [
    records, recordColumns, tableConfig.seq_column_width, recordTableWidth,
    recordWidthOverrides.overrides, recordActionWidth,
  ]);
  ...
  const doSubmit = async () => {
    // v2.3.19: 提交期间禁用按钮并忽略重复点击，避免整批记录被重复登记。
  ...
  const confirmSampleWorkload = async (data: { quantity: number; multiplier: number; method_id?: number; custom_method_name?: string; custom_instrument_name?: string; notes?: string }) => {
    if (!sampleWorkloadPreview) return;
    // v2.3.13: 缺少方法时明确提示，不再静默返回导致“点了确认却没反应”。
    // v2.3.14: 方法库没有候选时允许自定义方法/仪器，两条路径至少要有其一。
    if (!data.method_id && !data.custom_method_name) {
      setSnack({ open: true, msg: '请先选择方法 / 仪器，或填写自定义方法名称', sev: 'error' });
      throw new Error('请先选择方法 / 仪器');
    }
    const recordId = sampleWorkloadPreview.source_record_id;
    try {
      requireApiSuccess(await createSampleInfoWorkload(recordId, {
        method_id: data.method_id, custom_method_name: data.custom_method_name, custom_instrument_name: data.custom_instrument_name,
        quantity: data.quantity, multiplier: data.multiplier, notes: data.notes,
      }), '录入工作量失败');
      // 立即更新当前页，避免列表刷新期间按钮仍显示“录入工作量”。
      setRecords(prev => prev.map(record => record.id === recordId ? { ...record, workload_recorded: true } : record));
      setSampleWorkloadPreview(null);
      setSnack({ open: true, msg: '工作量录入成功', sev: 'success' });
      // 刷新后以服务端状态为准；若服务端仍未标记已录入，明确提示而不是给出假状态。
      const items = await load();
      const current = items.find(item => item.id === recordId);
      if (current && !current.workload_recorded) {
        setSnack({ open: true, msg: '工作量已写入，但列表状态未同步，请刷新页面后核对', sev: 'error' });
      }
    } catch (e: any) {
  ...
  return (
    ...
    <Box sx={{ width: '100%', maxWidth: 1536, mx: 'auto', mt: { xs: 1, md: 2 }, px: { xs: 0, sm: 1 } }}>
      ...
      {canCreate && <Paper elevation={0} sx={{ p: { xs: 1.25, md: 2 }, mb: 2, borderRadius: '6px', border: '1px solid #d9e2dc', borderTop: '3px solid #2e7d32', bgcolor: '#fff' }}>
        ...
        <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', sm: '1fr auto' }, gap: 1 }}>
          ...
          <Button
            ...
            sx={{ width: { xs: '100%', sm: 'auto' }, borderRadius: R, bgcolor: '#2e7d32', '&:hover': { bgcolor: '#1b5e20' } }}
          >
            {submitting ? '提交中…' : `提交登记（${rows.length} 行）`}
          </Button>
      ...
      <SampleInfoRecordList
        ...
        actionCollapsed={actionColumnMode === 'auto' && recordTableWidth < 1400}
        rowHeight={tableConfig.row_height}
        widthOverrides={recordWidthOverrides}
      ...
      <Dialog open={Boolean(returningRecord)} onClose={() => setReturningRecord(null)} fullWidth maxWidth="sm" disableRestoreFocus PaperProps={{ sx: { maxHeight: 'min(640px, calc(100vh - 32px))' } }}>
            ...
            <TableContainer component={Paper} variant="outlined" sx={{ borderRadius: R, mb: 1, overflowX: 'auto' }}>
              <Table size="small" sx={adaptiveTableSx}>
                ...
                <TableBody>
                  {records.length === 0 ? (
                    <TableRow>
                      <TableCell colSpan={legacyListColumnCount} align="center" sx={{ py: 4, color: '#999', fontSize: '0.875rem' }}>暂无登记记录</TableCell>
                    </TableRow>
                  ) : records.map(r => {
                    ...
                    return (
                      <React.Fragment key={r.id}>
                        ...
                        >
                          ...
                          <TableCell sx={{ ...listColumnCellSx('seq_no'), fontSize: '0.875rem', color: '#999', borderColor: '#e0e0e0' }}>#{r.seq_no}{showBusinessNo && r.business_no && <Box sx={{ mt: 0.25, fontFamily: 'monospace', fontSize: '0.62rem', lineHeight: 1.2, overflowWrap: 'anywhere' }}>{r.business_no}</Box>}</TableCell>
                          {listColumns.filter(c => !['status', 'seq_no'].includes(c.field_key)).map(col => (
                        ...
                        {expandedId === r.id && (
                          <TableRow>
                            <TableCell colSpan={legacyListColumnCount} sx={{ p: 0, border: 'none' }}>
