...
/// JWT 签名密钥。优先使用 `JWT_SECRET` 环境变量；未配置时使用数据目录下自动生成并持久化的密钥文件。
/// v2.3.19：移除随安装包分发的固定默认密钥。旧密钥写在二进制里，任何人都能用它离线签发
/// 管理员 Token，属于无法通过改密撤销的后门。
fn jwt_secret() -> Result<String> {
    ...
    let secret = match std::env::var("JWT_SECRET") {
        ...
        _ => load_or_create_jwt_secret()?,
    };
    let _ = CACHE.set(secret.clone());
    Ok(secret)
}

/// 每套部署生成一次随机密钥并落盘，重启和升级后复用，避免已登录会话被动全部失效。
fn load_or_create_jwt_secret() -> Result<String> {
    let path = AppConfig::load().data_dir().join("jwt_secret");
    ...
    std::fs::write(&path, &secret)
        .map_err(|error| AppError::Internal(format!("写入 JWT 密钥文件失败: {error}")))?;
    Ok(secret)
}

/// 启动阶段预生成密钥；数据目录不可写时立即失败，而不是等到第一次登录才发现。
...
/// v2.3.19 登录失败限流：同一账号连续失败 5 次后锁定 15 分钟。
/// ponytail: 进程内计数，重启即清空；当前为单实例部署，多实例需改为共享存储。
const LOGIN_MAX_FAILURES: u32 = 5;
const LOGIN_LOCK_MINUTES: i64 = 15;

type LoginFailures =
    std::sync::Mutex<std::collections::HashMap<String, (u32, chrono::DateTime<Utc>)>>;

fn login_failures() -> &'static LoginFailures {
    static FAILURES: OnceLock<LoginFailures> = OnceLock::new();
    FAILURES.get_or_init(|| std::sync::Mutex::new(std::collections::HashMap::new()))
}

fn login_lock_remaining(username: &str) -> Option<i64> {
    let mut guard = login_failures().lock().ok()?;
    let (count, last) = *guard.get(username)?;
    if count < LOGIN_MAX_FAILURES {
        return None;
    }
    let until = last + Duration::minutes(LOGIN_LOCK_MINUTES);
    let now = Utc::now();
    if now >= until {
...
fn record_login_failure(username: &str) {
    if let Ok(mut guard) = login_failures().lock() {
        let entry = guard.entry(username.to_string()).or_insert((0, Utc::now()));
        entry.0 += 1;
        entry.1 = Utc::now();
    }
}
...
pub fn login(pool: &DbPool, req: &LoginRequest) -> Result<LoginResponse> {
    if let Some(minutes) = login_lock_remaining(&req.username) {
        return Err(AppError::Validation(format!(
            "登录失败次数过多，请在 {minutes} 分钟后重试"
        )));
    }
    let user = match user_repo::find_by_username(pool, &req.username)? {
        Some(user) => user,
        None => {
