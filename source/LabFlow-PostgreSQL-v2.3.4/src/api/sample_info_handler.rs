...
fn workload_instruments(
    ...
    let mut stmt = conn.prepare(
        "SELECT DISTINCT m.id,i.id,
                COALESCE(NULLIF(i.code,''),i.name) || ' · ' || COALESCE(NULLIF(m.full_name,''),m.name),
                m.multiplier
         FROM projects p
         JOIN project_lab_links pll ON pll.project_id=p.id
         JOIN project_groups pg ON pg.id=pll.group_id
         JOIN project_method_links pml ON pml.project_id=p.id
         JOIN methods m ON m.id=pml.method_id
         JOIN instruments i ON i.id=m.instrument_id
         LEFT JOIN method_type_links mtl ON mtl.method_id=m.id
         LEFT JOIN method_types mt ON mt.id=mtl.method_type_id
         WHERE p.name=?1 AND pg.name=?2
           AND (mt.name=?3 OR p.method_type=?3)
           AND p.deleted_at IS NULL AND p.is_active=1
           AND m.deleted_at IS NULL AND m.is_active=1 AND m.show_in_sample_info=1
           AND i.deleted_at IS NULL AND i.is_active=1
         ORDER BY COALESCE(NULLIF(i.code,''),i.name),m.id",
    )?;
    let rows = stmt.query_map(
        postgres_compat::params![&record.project_name, &record.lab_name, &record.detection_type],
        |row| Ok(SampleInfoWorkloadInstrument {
            method_id: row.get(0)?,
...
async fn sample_workload_preview(
    ...
    let department = record.division_id
    ...
    Ok(Json(ApiResponse::ok(SampleInfoWorkloadPreview {
        ...
        instruments: workload_instruments(&pool, &record)?,
        recorded: crate::repo::record_repo::exists_source(&pool, "sample_info_sample", id)?,
    })))
}
...
async fn sample_workload_confirm(
    ...
    if crate::repo::record_repo::exists_source(&pool, "sample_info_sample", id)? {
        return Err(AppError::Validation("本次取样已录入工作量".into()));
    }
    let candidate = workload_instruments(&pool, &record)?
        .into_iter()
        .find(|item| item.method_id == body.method_id)
        .ok_or_else(|| AppError::Validation("所选仪器不属于该项目、实验室和检测类型".into()))?;
    let subject = crate::repo::user_repo::find_by_username(&pool, &record.sampled_by)?
        .ok_or_else(|| AppError::Validation("取样人员不存在，无法归属工作量".into()))?;
    let project_and_group = {
        let conn = pool.get()?;
        conn.query_row(
            "SELECT p.id,pg.id FROM projects p
             JOIN project_lab_links pll ON pll.project_id=p.id
             JOIN project_groups pg ON pg.id=pll.group_id
             WHERE p.name=?1 AND pg.name=?2 AND p.deleted_at IS NULL AND p.is_active=1
             ORDER BY p.id LIMIT 1",
            postgres_compat::params![&record.project_name, &record.lab_name],
            |row| Ok((row.get::<_, i64>(0)?, row.get::<_, i64>(1)?)),
        )?
    };
    let workload = crate::models::record::RecordCreate {
        project_id: project_and_group.0,
        method_id: Some(candidate.method_id),
        user_name: subject.username,
        sender_user_id: Some(subject.id),
        quantity: body.quantity,
        recorded_at: chrono::Utc::now()
            .with_timezone(&chrono::FixedOffset::east_opt(8 * 60 * 60).expect("valid UTC+8"))
            .format("%Y-%m-%dT%H:%M:%S").to_string(),
        group_id: Some(project_and_group.1),
        multiplier: Some(body.multiplier),
        high_item: None,
        division_id: record.division_id,
        extra_fields: Some(serde_json::json!({"source_business_no": record.business_no, "source_notes": body.notes.unwrap_or_default(), "source_instrument_id": candidate.instrument_id})),
        source_type: Some("sample_info_sample".into()),
        source_record_id: Some(id),
    };
    Ok(Json(ApiResponse::ok(crate::service::record_service::create_record(
        &pool, &workload, &ctx.user.username,
    )?)))
}
...
async fn withdraw_sample(
    ...
    ensure_record_access(&pool, &ctx, &existing)?;
    if crate::repo::record_repo::exists_source(&pool, "sample_info_sample", id)? {
        return Err(AppError::Validation("该取样已录入工作量，请先按工作量记录流程处理".into()));
    }
