...
async fn sample_workload_preview(
    ...
    ensure_rd_record_department_access(&pool, &ctx, &record)?;
    if record.status != "已取样" || record.sampled_at.is_none() {
        return Err(AppError::Validation("请先完成取样后再录入工作量".into()));
    }
    let method_id = record.method_id.ok_or_else(|| AppError::Validation("该记录未关联检测方法".into()))?;
    ...
    Ok(Json(ApiResponse::ok(SampleWorkloadPreview {
        ...
        recorded: crate::repo::record_repo::exists_source(&pool, "rd_sample", id)?,
    })))
}
...
async fn sample_workload_confirm(
    ...
    let subject = crate::repo::user_repo::find_by_username(&pool, record.sampler.as_deref().unwrap_or_default())?
    ...
    let workload = crate::models::record::RecordCreate {
        ...
        extra_fields: Some(serde_json::json!({"source_business_no": record.business_no, "source_notes": body.notes.unwrap_or_default()})),
