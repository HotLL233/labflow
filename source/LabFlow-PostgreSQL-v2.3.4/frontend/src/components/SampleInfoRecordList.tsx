...
export default function SampleInfoRecordList(props: SampleInfoRecordListProps) {
  ...
  const RecordActions = ({ record }: { record: SampleInfoRecord }) => (
    <Box sx={{
      ...
      {record.status === STATUS_WAIT_TEST && hasPermission('sample-info:record-workload') && <Button size="small" variant="outlined" color="primary" onClick={event => { event.stopPropagation(); onRecordWorkload(record.id); }} sx={{ borderRadius: R, whiteSpace: 'nowrap' }}>录入工作量</Button>}
  ...
  const ColumnValue = ({ record, column }: { record: SampleInfoRecord; column: SampleInfoColumn }) => {
    ...
    if (column.field_key === 'seq_no') return <><Box sx={{ fontWeight: 700 }}>#{record.seq_no ?? '—'}</Box><Box sx={{ color: 'text.secondary', fontSize: '0.68rem', overflowWrap: 'anywhere' }}>{valueText(record.business_no)}</Box></>;
    if (['submitted_at', 'detection_date', 'sampled_at'].includes(column.field_key)) {
      return <>{formatDate(String(getRecordValue(record, column.field_key) || ''))}</>;
    }
    return <>{valueText(getRecordValue(record, column.field_key))}</>;
  };
  ...
  return (
    <Paper elevation={0} sx={{ p: { xs: 1, sm: 1.5, lg: 2 }, borderRadius: R, border: '1px solid #e0e0e0' }}>
      ...
      {loading ? <Box sx={{ textAlign: 'center', py: 4 }}><CircularProgress size={32} /></Box> : (
        <>
          <TableContainer className={extraClass} sx={{ display: { xs: 'none', lg: 'block' }, width: '100%', overflowX: 'auto', border: '1px solid #e5e5e5', borderRadius: R }}>
                ...
                {records.length === 0 ? <TableRow><TableCell colSpan={Math.max(2, columns.length + 1)} align="center" sx={{ py: 4, color: 'text.secondary' }}>暂无登记记录</TableCell></TableRow> : records.map(record => (
                  <React.Fragment key={record.id}>
                    <TableRow hover sx={{ bgcolor: record.status === STATUS_DONE ? '#fbfbfb' : '#fff', verticalAlign: 'top', '& td': { px: 0.8, py: 1 } }}>
                      {columns.map(column => <TableCell key={column.field_key} sx={recordCellSx}><ColumnValue record={record} column={column} /></TableCell>)}
                      <TableCell sx={{ ...recordCellSx, px: 0.5, textAlign: 'center', whiteSpace: 'nowrap' }}><RecordActions record={record} /></TableCell>
                    </TableRow>
