...
fn inserted_table_name(sql: &str) -> Option<String> {
    static INSERT_TABLE: OnceLock<Regex> = OnceLock::new();
    let pattern = INSERT_TABLE.get_or_init(|| {
        Regex::new(
            r#"(?is)^\s*INSERT\s+INTO\s+(?:(?:"([^"]+)"|([A-Za-z_][A-Za-z0-9_$]*))\s*\.\s*)?(?:"([^"]+)"|([A-Za-z_][A-Za-z0-9_$]*))\b"#,
        )
        .expect("insert table expression is valid")
    });
    ...
    let table = captures.get(3).or_else(|| captures.get(4))?.as_str();
    Some(match schema {
...
fn has_returning_clause(sql: &str) -> bool {
    ...
    RETURNING
        .get_or_init(|| Regex::new(r"(?is)\bRETURNING\b").expect("returning expression is valid"))
        .is_match(sql)
}
...
impl Connection {
    ...
    pub fn execute<P: Params>(&self, sql: &str, params: P) -> Result<usize> {
        ...
        let insert_table = if insert {
            inserted_table_name(&sql)
        } else {
            None
        };
        let (affected, last_id) = self.run(move |client| {
            ...
            if let Some(table) = insert_table {
                let id_sequence: Option<String> = client
                    .query_one(
                        "SELECT CASE
                            WHEN EXISTS (
                                SELECT 1
                                ...
                                WHERE attrelid = to_regclass($1)
                                  ...
                                  AND attnum > 0
                            ) THEN pg_get_serial_sequence($1, 'id')
                            ELSE NULL
                        END",
                        &[&table],
                    )?
                    .get(0);
                if id_sequence.is_some() && !has_returning_clause(&sql) {
                    let returning_sql =
                        format!("{} RETURNING id", sql.trim_end().trim_end_matches(';'));
                    let rows = client.query(&returning_sql, &params)?;
                    let last_id = rows
                        .last()
                        .map(|row| row.try_get::<_, i64>(0))
                        .transpose()?;
                    return Ok((rows.len(), last_id));
                }
            }
...
impl<'a> Transaction<'a> {
    ...
    /// Explicitly roll back a failed operation so the connection is never
    /// returned to the pool while PostgreSQL still considers the transaction aborted.
...
impl<'a> Statement<'a> {
    fn fetch<P: Params>(&mut self, params: P) -> Result<Vec<Row<'static>>> {
        ...
        self.connection.run(move |client| {
            ...
            rows.into_iter().map(Row::from_pg).collect()
        })
    }
...
impl ConnectionManager {
    ...
    pub fn test_database() -> Result<Self> {
        ...
        let database_url = std::env::var("WORKLOAD_TEST_DATABASE_URL").map_err(|_| {
            Error::Unsupported(
                "WORKLOAD_TEST_DATABASE_URL is required for PostgreSQL integration tests".into(),
            )
        })?;
...
mod tests {
    ...
    fn converts_legacy_char_function_without_touching_to_char() {
        let sql =
            convert_sql("SELECT m.name || char(31) || i.code, to_char(CURRENT_TIMESTAMP, 'YYYY')");
