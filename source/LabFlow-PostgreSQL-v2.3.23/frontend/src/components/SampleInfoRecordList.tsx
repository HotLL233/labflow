        ...
        <>
          <TableContainer ref={containerRef} className={extraClass} sx={{ display: { xs: 'none', lg: 'block' }, width: '100%', maxHeight: '72vh', overflow: 'auto', border: '1px solid #e5e5e5', borderRadius: R }}>
            <Table size="small" stickyHeader sx={{
              width: `max(100%, ${visibleColumns.reduce((sum, column) => sum + (columnWidths[column.field_key]?.mobile || 0), 0) + (columnWidths._action?.mobile || 0)}px)`,
              minWidth: `${visibleColumns.reduce((sum, column) => sum + (columnWidths[column.field_key]?.mobile || 0), 0) + (columnWidths._action?.mobile || 0)}px`,
              maxWidth: 'none',
              tableLayout: 'fixed',
              ...stickyHeaderSx,
            }}>
