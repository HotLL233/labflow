...
import {
  Box, Button, Checkbox, Chip, Divider, FormControlLabel, IconButton, ListItemText, Menu, MenuItem,
  Stack, TextField, Tooltip, Typography,
} from '@mui/material';
import { getSetting } from '../../api/admin';
...
/**
 * v2.3.20：操作列收纳开关。
 * `auto`（默认）按容器宽度在文字档 / 图标档 / 收纳档之间自动切换；
 * `full` 始终显示全部按钮，等同升级前行为。
 */
export function useActionColumnMode(): 'auto' | 'full' {
  const [mode, setMode] = useState<'auto' | 'full'>('auto');
  useEffect(() => {
    let cancelled = false;
    getSetting('record_table_action_mode')
      .then(response => {
        if (cancelled || response.code !== 0) return;
        const value = (response.data as { value?: string } | null)?.value;
        if (value === 'full') setMode('full');
      })
      .catch(() => {
        // 读取失败时按默认的自动收纳处理，不阻塞列表展示。
      });
    return () => {
      cancelled = true;
    };
...
export const ColumnSettingsButton: React.FC<{
  ...
  onReset: () => void;
  /** v2.3.20：本机已调整的列宽数量，>0 时显示「恢复默认宽度」 */
  widthOverrideCount?: number;
  onResetWidths?: () => void;
}> = ({ columns, hidden, onToggle, onReset, widthOverrideCount = 0, onResetWidths }) => {
  ...
  return (
    <>
      ...
      <Menu anchorEl={anchor} open={!!anchor} onClose={() => setAnchor(null)}>
        ...
        {/* v2.3.20：列宽来源提示与恢复入口。列宽默认由后台配置与内容测量决定，
            用户拖动表头分隔线后产生本机覆盖，优先级最高。 */}
        <Divider />
        <Box sx={{ px: 1.5, py: 0.5, display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 1 }}>
          <Typography variant="caption" color="text.secondary">
            {widthOverrideCount > 0 ? `宽度：本机已调整 ${widthOverrideCount} 列` : '宽度：跟随后台'}
          </Typography>
          {widthOverrideCount > 0 && onResetWidths && (
            <Button size="small" onClick={onResetWidths}>恢复默认宽度</Button>
          )}
        </Box>
        <Divider />
        {columns.map(column => (
