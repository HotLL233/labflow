...
const RdRecordsPage: React.FC = () => {
  ...
  const tableContentWidth = Object.values(columnLayouts).reduce((sum, layout) => sum + layout.px, 0);
              ...
              {filteredRecords.map((rec, idx) => {
                ...
                return (
                  <TableRow key={rec.id} hover selected={selectedIds.includes(rec.id)} sx={{ '&:last-child td': { borderBottom: 0 }, verticalAlign: 'top', '& > td': { minHeight: `${tableConfig.row_height}px` } }}>
                    ...
                    <TableCell sx={[recordCellSx, cellWidthSx('actions'), { px: 0.5, textAlign: 'center', whiteSpace: 'nowrap', height: tableConfig.row_height }]}>
                      {isEditing ? (
