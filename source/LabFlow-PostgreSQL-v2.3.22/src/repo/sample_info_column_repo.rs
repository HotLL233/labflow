    ...
    let mut stmt = conn.prepare(
        "SELECT c.id, c.field_key, c.label, c.data_type, c.is_predefined, c.is_required, c.is_active, \
         ...
         c.type_key, c.created_at, c.updated_at, \
          ...
          c.width_mode, c.min_width, c.max_width \
    ...
    let rows = stmt.query_map([], |row| {
        ...
        Ok(SampleInfoColumn {
            ...
            required_types: rt
                ...
                .map(|s| s.to_string())
                .collect(),
            // v2.3.20：老库升级后新列已由迁移补齐；此处兜底避免读路径失败。
            width_mode: row
                .get::<_, String>(18)
    ...
    let rows = stmt.query_map([type_key], |row| {
        Ok(SampleInfoColumn {
            ...
            width_mode: row
                .get::<_, String>(17)
                .unwrap_or_else(|_| "auto".to_string()),
            min_width: row.get::<_, i64>(18).unwrap_or(0),
            max_width: row.get::<_, i64>(19).unwrap_or(0),
            visible_types: vec![],
            required_types: vec![],
        })
    })?;
...
pub fn list_all_with_visibility(
    ...
    let sql = "
        SELECT c.id, c.field_key, c.label, c.data_type, c.is_predefined, c.is_required, c.is_active,
               ...
               COALESCE((SELECT string_agg(v2.type_key, ',' ORDER BY v2.type_key) FROM sample_info_column_visibility v2 WHERE v2.column_id=c.id AND v2.is_visible=1), '') AS visible_types,
               COALESCE((SELECT string_agg(v3.type_key, ',' ORDER BY v3.type_key) FROM sample_info_column_visibility v3 WHERE v3.column_id=c.id AND v3.is_visible=1 AND v3.is_required=1), '') AS required_types,
               c.width_mode, c.min_width, c.max_width
        FROM sample_info_columns c
        LEFT JOIN sample_info_column_visibility v ON v.column_id = c.id AND v.type_key = ?1
    ...
    let mut stmt = conn.prepare(sql)?;
    let rows = stmt.query_map([type_key], |row| {
        let visible_types: String = row.get(18).unwrap_or_default();
        let required_types: String = row.get(19).unwrap_or_default();
        let col = SampleInfoColumn {
            ...
            width_mode: row
                .get::<_, String>(20)
                .unwrap_or_else(|_| "auto".to_string()),
            min_width: row.get::<_, i64>(21).unwrap_or(0),
            max_width: row.get::<_, i64>(22).unwrap_or(0),
...
pub fn create(pool: &DbPool, data: &ColumnCreate, operator: &str) -> Result<SampleInfoColumn> {
    ...
    // v2.3.20：列宽模式，未传时按 auto 处理（由前端按内容测量）。
    let width_mode = normalize_width_mode(data.width_mode.as_deref().unwrap_or("auto"))?;
    let min_width = normalize_custom_bound(data.min_width.unwrap_or(0))?;
    let max_width = normalize_custom_bound(data.max_width.unwrap_or(0))?;
    ...
    tx.execute(
        "INSERT INTO sample_info_columns (field_key, label, data_type, type_key, is_predefined, is_required, is_active, width, width_mode, min_width, max_width, sort_order, options, show_in_list, show_in_export, show_in_form) \
         VALUES (?1, ?2, ?3, ?4, 0, ?5, 1, ?6, ?7, ?8, ?9, ?10, ?11, ?12, ?13, ?14)",
        params![
            field_key, label, data_type, data.type_key,
            is_required as i64, width, width_mode, min_width, max_width, sort_order, data.options,
...
pub fn update(
    ...
    // v2.3.20：未传时保留原值，避免部分更新把「自定义」意外重置为「自动」。
    ...
    tx.execute(
        "UPDATE sample_info_columns SET label=?1, data_type=?2, is_active=?3, is_required=?4, \
         width=?5, width_mode=?6, min_width=?7, max_width=?8, options=?9, show_in_list=?10, \
         show_in_export=?11, show_in_form=?12, \
         updated_at=datetime('now','localtime') WHERE id=?13",
        params![
            label,
            data_type,
            is_active,
