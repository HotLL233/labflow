...
pub fn run(connection: &postgres_compat::Connection, initial_admin_password: &str) -> Result<()> {
    ...
    let sample_info_action_columns_v2_migrated: bool = connection.query_row(
        "SELECT EXISTS(SELECT 1 FROM schema_migrations WHERE version='2.3.12-sample-info-action-columns')",
        [],
        |row| row.get(0),
    )?;
    if !sample_info_action_columns_v2_migrated {
        connection.execute_batch(include_str!(
            "postgres_v2_3_12_sample_info_action_columns.sql"
        ))?;
    }
    let sample_info_action_columns_migrated: bool = connection.query_row(
        "SELECT EXISTS(SELECT 1 FROM schema_migrations WHERE version='2.3.5-sample-info-action-columns')",
        [],
        |row| row.get(0),
    )?;
    if !sample_info_action_columns_migrated {
        connection.execute_batch(include_str!(
            "postgres_v2_3_5_sample_info_action_columns.sql"
        ))?;
    }
    ...
    let v2_3_6_workload_permissions_migrated: bool = connection.query_row(
        "SELECT EXISTS(SELECT 1 FROM schema_migrations WHERE version='2.3.6-workload-permissions-and-action-form')",
        [],
        |row| row.get(0),
    )?;
    if !v2_3_6_workload_permissions_migrated {
        connection.execute_batch(include_str!(
            "postgres_v2_3_6_workload_permissions_and_action_form.sql"
        ))?;
    }
    let v2_3_7_workload_permission_repair_migrated: bool = connection.query_row(
        "SELECT EXISTS(SELECT 1 FROM schema_migrations WHERE version='2.3.7-workload-permission-whitelist-and-role-repair')",
        [],
        |row| row.get(0),
    )?;
    if !v2_3_7_workload_permission_repair_migrated {
        connection.execute_batch(include_str!(
            "postgres_v2_3_7_workload_permission_whitelist_and_role_repair.sql"
        ))?;
    }
    let v2_3_8_sample_workload_operator_ownership_migrated: bool = connection.query_row(
        "SELECT EXISTS(SELECT 1 FROM schema_migrations WHERE version='2.3.8-sample-workload-operator-ownership')",
        [],
        |row| row.get(0),
    )?;
    if !v2_3_8_sample_workload_operator_ownership_migrated {
        connection.execute_batch(include_str!(
            "postgres_v2_3_8_sample_workload_operator_ownership.sql"
        ))?;
    }
    ...
    let v2_3_18_sample_workload_quantity_migrated: bool = connection.query_row(
        "SELECT EXISTS(SELECT 1 FROM schema_migrations WHERE version='2.3.18-sample-workload-quantity')",
        [],
        |row| row.get(0),
    )?;
    if !v2_3_18_sample_workload_quantity_migrated {
        connection.execute_batch(include_str!(
            "postgres_v2_3_18_sample_workload_quantity.sql"
        ))?;
    }
    let v2_3_20_column_width_mode_migrated: bool = connection.query_row(
        "SELECT EXISTS(SELECT 1 FROM schema_migrations WHERE version='2.3.20-column-width-mode')",
        [],
        |row| row.get(0),
    )?;
    if !v2_3_20_column_width_mode_migrated {
        connection.execute_batch(include_str!("postgres_v2_3_20_column_width_mode.sql"))?;
    }
...
mod tests {
    ...
    fn sample_workload_migration_adds_deduplicated_source_columns() {
        ...
        let source_index_exists: bool = conn
            .query_row(
                "SELECT to_regclass('idx_work_records_sample_source') IS NOT NULL",
                [],
                |row| row.get(0),
            )
            .expect("source lookup index");
        assert!(source_index_exists);
    }

    #[test]
    fn workload_permission_migration_repairs_analysis_roles() {
        if std::env::var("WORKLOAD_TEST_DATABASE_URL").is_err() {
            return;
        }
        let conn = Connection::open_test_database().expect("PostgreSQL test database");
        ...
        for (role_name, permission_key) in [
            ...
            let exists: bool = conn
                .query_row(
                    ...
                    postgres_compat::params![role_name, permission_key],
                    |row| row.get(0),
                )
                .expect("role permission state");
            assert!(exists, "missing {permission_key} for {role_name}");
        }
