...
const SampleInfoStatsPage: React.FC = () => {
  ...
  const ldRecs = useCallback(async (pg = 1) => {
    ...
    try {
      ...
      if (r.code === 0 && r.data) { setRecords(r.data.items); setRecTotal(r.data.total); setRecPage(pg); }
      else setEr(r.message || '加载记录失败');
    } catch (e: any) { setEr(e?.message || '加载记录失败'); } finally { setRecLd(false); }
  }, [si, ei, recFilter, statusFilter, ownershipBasis, divisionFilter, includePendingOwnership]);
  ...
  const doExport = async () => {
    try {
      // v2.3.19: 导出必须与列表使用同一组筛选条件，此前漏传 status，
      // 会出现「页面只看待取样、导出却包含全部状态」。
      await exportSampleInfo({
        start: si,
        end: ei,
        type_key: recFilter || undefined,
        status: statusFilter || undefined,
          ...
          {recLd ? <Box sx={{ textAlign: 'center', py: 4 }}><CircularProgress size={24} /></Box> : (
            <>
              <TableContainer component={Paper} sx={{ ...tablePaperSx, overflowX: "auto" }}>
                <Table size="small" sx={{ ...adaptiveTableSx, '& tbody td': recordValueCellSx }}>
                  <TableHead><TableRow>
