...
fn list_all_on_conn(conn: &postgres_compat::Connection) -> Result<Vec<RdRecordColumn>> {
    let mut stmt = conn.prepare(
        "SELECT id,name,label,data_type,width,sort_order,is_predefined,is_required,is_active,\
         show_in_list,show_in_form,show_in_export,options,option_detail_rules,default_value,placeholder,applicable_types,entry_row,created_at,updated_at \
...
pub fn create(
    ...
    tx.execute(
        "INSERT INTO rd_record_columns(name,label,data_type,width,sort_order,is_predefined,is_required,is_active,show_in_list,show_in_form,show_in_export,options,option_detail_rules,default_value,placeholder,applicable_types,entry_row,created_at,updated_at) \
         VALUES(?1,?2,?3,?4,?5,0,?6,1,?7,?8,?9,?10,?11,?12,?13,?14,?15,datetime('now','localtime'),datetime('now','localtime'))",
        postgres_compat::params![name,label,data.data_type.trim(),width,next_sort,data.is_required as i64,data.show_in_list as i64,data.show_in_form as i64,data.show_in_export as i64,data.options.trim(),option_detail_rules,data.default_value.trim(),data.placeholder.trim(),data.applicable_types.trim(),data.entry_row],
    )?;
...
pub fn update(
    ...
    let tx = conn.transaction()?;
    tx.execute(
        "UPDATE rd_record_columns SET label=?1,data_type=?2,width=?3,sort_order=?4,is_required=?5,is_active=?6,show_in_list=?7,show_in_form=?8,show_in_export=?9,options=?10,option_detail_rules=?11,default_value=?12,placeholder=?13,applicable_types=?14,entry_row=?15,updated_at=datetime('now','localtime') WHERE id=?16",
