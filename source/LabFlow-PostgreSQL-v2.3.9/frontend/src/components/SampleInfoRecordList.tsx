...
export default function SampleInfoRecordList(props: SampleInfoRecordListProps) {
  ...
  const actionButtonSx = { borderRadius: R, whiteSpace: 'nowrap' as const };
  ...
  const renderConfiguredAction = (record: SampleInfoRecord, fieldKey: string) => {
    if (fieldKey === 'action_edit') {
      ...
      return allowed ? <Button size="small" variant="outlined" startIcon={<EditIcon />} onClick={event => { event.stopPropagation(); onEdit(record); }} sx={actionButtonSx}>编辑</Button> : null;
    }
    if (fieldKey === 'action_record_workload') {
      if (record.workload_recorded) {
        return <Button size="small" variant="outlined" color="success" disabled sx={actionButtonSx}>已录入</Button>;
      }
      return record.status === STATUS_WAIT_TEST && hasPermission('sample-info:record-workload')
        ? <Button size="small" variant="outlined" color="primary" onClick={event => { event.stopPropagation(); onRecordWorkload(record.id); }} sx={actionButtonSx}>录入工作量</Button>
        : null;
    }
    if (fieldKey === 'action_return') {
      if (record.status === STATUS_WAIT_SAMPLE && hasPermission('sample-info:return')) {
        return <Button size="small" variant="outlined" color="secondary" startIcon={<UndoIcon />} onClick={event => { event.stopPropagation(); onReturn(record); }} sx={actionButtonSx}>退回</Button>;
      }
      if (record.status === STATUS_RETURNED && hasPermission('sample-info:return-confirm') && (isSystemAdmin || record.created_by_user_id === currentUserId)) {
        return <Button size="small" variant="outlined" color="primary" startIcon={<UndoIcon />} onClick={event => { event.stopPropagation(); onConfirmReturn(record); }} sx={{ ...actionButtonSx, whiteSpace: 'normal' }}>确认退回</Button>;
      }
    }
    return null;
  };

  const ConfiguredActions = ({ record }: { record: SampleInfoRecord }) => (
  ...
  const ColumnValue = ({ record, column }: { record: SampleInfoRecord; column: SampleInfoColumn }) => {
    if (column.data_type === 'action') return renderConfiguredAction(record, column.field_key);
          ...
          <TableContainer className={extraClass} sx={{ display: { xs: 'none', lg: 'block' }, width: '100%', overflowX: 'auto', border: '1px solid #e5e5e5', borderRadius: R }}>
            <Table size="small" sx={{ width: 'max-content', minWidth: '100%', tableLayout: 'fixed' }}>
              ...
              <TableHead><TableRow sx={{ bgcolor: '#f5f7f5' }}>
                ...
                <TableCell sx={{ fontWeight: 700, whiteSpace: 'normal', overflowWrap: 'break-word', wordBreak: 'normal', px: 0.8, py: 1, borderColor: '#e0e0e0', minWidth: 0 }}>其他操作</TableCell>
              </TableRow></TableHead>
              <TableBody>
              ...
              <Card key={record.id} variant="outlined" sx={{ borderRadius: R, borderLeft: `4px solid ${record.status === STATUS_WAIT_SAMPLE ? '#d32f2f' : record.status === STATUS_WAIT_TEST ? '#f9a825' : '#2e7d32'}` }}>
                <CardContent sx={{ p: 1.25, '&:last-child': { pb: 1.25 } }}>
                  ...
                  {editable(record) ? <Box onClick={event => event.stopPropagation()} onMouseDown={event => event.stopPropagation()} sx={{ display: 'grid', gap: 1 }}>
                    ...
                    <Box sx={{ mt: { xs: 1.75, lg: 1 }, display: 'grid', gap: 0.75 }}><ConfiguredActions record={record} /><RecordActions record={record} /></Box>
                  </>}
                </CardContent>
              </Card>
            ))}
