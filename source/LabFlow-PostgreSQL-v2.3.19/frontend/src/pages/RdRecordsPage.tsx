...
const getRecordColumnBounds = (name: string, configuredWidth?: number) => {
  const bounds: Record<string, { min?: number; max?: number; fixed?: number }> = {
    ...
    // v2.3.18: 撤回取样按钮移入本列，取样人列需要容纳按钮宽度。
    sampling_person: { min: 96, max: 128 },
  ...
  return (
    <Box>
      ...
      {records.length === 0 ? (
        ...
        <TableContainer component={Paper} variant="outlined" sx={{ borderRadius: R, boxShadow: 'none', maxHeight: '72vh', overflow: 'auto' }}>
          <Table size="small" stickyHeader sx={{ ...adaptiveTableSx, ...stickyHeaderSx }}>
            <TableHead>
              <TableRow>
                <TableCell padding="checkbox" sx={{ width: 40, px: 0.5, py: 0.5, textAlign: 'center', fontWeight: 700 }}>
                  <Checkbox
                    size="small"
              ...
              {filteredRecords.map((rec, idx) => {
                ...
                return (
                  <TableRow key={rec.id} hover selected={selectedIds.includes(rec.id)} sx={{ '&:last-child td': { borderBottom: 0 }, verticalAlign: 'top' }}>
                    <TableCell padding="checkbox" sx={{ width: 40, px: 0.5, textAlign: 'center' }}>
                      <Checkbox
                        size="small"
