...
const SampleInfoEntry: React.FC = () => {
  ...
  const recordDisplayWidths = useMemo(() => {
    ...
    const configuredWidth = (column: SampleInfoColumn, fallback: number) => {
      const width = Number(column.width);
      return Number.isFinite(width) && width > 0 ? Math.max(48, Math.min(width, 500)) : fallback;
    };

    displayColumns.forEach(column => {
      const fallback = column.data_type === 'action' ? 112 : column.data_type === 'attachment' ? 180 : 120;
      const width = column.field_key === 'seq_no'
        ? Math.max(tableConfig.seq_column_width || 54, 74)
        : configuredWidth(column, fallback);
      rawWidths[column.field_key] = width;
    });
    actionColumns.forEach(column => { rawWidths[column.field_key] = configuredWidth(column, 96); });
    const configuredActionWidth = actionColumns.reduce((sum, column) => sum + configuredWidth(column, 96), 0)
      + Math.max(0, actionColumns.length - 1) * 8;
    // v2.3.13: 操作列宽度按各操作按钮配置宽度求和，不再钳制到 520，
    // 让“取样 / 撤回取样 / 完成检测”等按钮能按后台配置的宽度显示。
    rawWidths._action = Math.max(180, Math.min(1600, configuredActionWidth + 24));
    const totalWidth = Object.values(rawWidths).reduce((sum, width) => sum + width, 0) || 1;
    const widths: Record<string, { desktop: string; mobile: number }> = {};
    Object.entries(rawWidths).forEach(([key, width]) => {
      widths[key] = { desktop: `${((width / totalWidth) * 100).toFixed(4)}%`, mobile: width };
    });
    return widths;
  }, [recordColumns, tableConfig.seq_column_width]);
