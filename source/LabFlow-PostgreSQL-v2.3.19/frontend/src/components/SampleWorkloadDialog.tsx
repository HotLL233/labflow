...
export type SampleWorkloadInstrument = { method_id: number; instrument_id: number; instrument: string; method_name?: string; multiplier: number; match_source?: 'record' | 'type' };
export type SampleWorkloadPreview = {
  ...
  recorded_quantity?: number; remaining_quantity?: number; coefficient?: number;
...
export default function SampleWorkloadDialog({ preview, onClose, onConfirm }: {
  ...
  // 研发送样不返回候选列表，直接使用方法/仪器摘要；样品信息登记优先使用精准命中的方法库候选。
  // v2.3.15: 候选已按“记录已填 → 检测类型”分级过滤；检索不到时才切换到自定义录入。
  ...
  useEffect(() => {
    if (!preview) return;
    const only = instruments.length === 1 ? instruments[0] : undefined;
    setMethodId(only?.method_id ?? '');
    setQuantity(preview.remaining_quantity ?? preview.quantity);
    setMultiplier(only?.multiplier ?? preview.multiplier ?? 1);
    setNotes('');
    setCustomMethod('');
    setCustomInstrument('');
  }, [preview, instruments]);
  ...
  const summary: Array<[string, string | number | undefined]> = [
    ...
    ['项目', preview?.project],
    ['检测类型', preview?.detection_type || '未提供'],
    ['方法 / 仪器', customMode ? (customMethod.trim() || '待自定义') : (selected?.instrument || (instruments.length > 0 ? '待选择' : preview?.instrument || preview?.method))],
    ['原始数量', preview?.quantity],
    ['已录入数量', preview?.recorded_quantity ?? 0],
    ['剩余数量', preview?.remaining_quantity ?? preview?.quantity],
  ];
      ...
      {preview?.recorded ? <Alert severity="info" sx={{ mb: 1.5 }}>本次取样已录入工作量。</Alert> : <>
        ...
        {customMode ? <>
          ...
          {instruments.length > 0 && <Box sx={{ mb: 1.5 }}><Typography variant="caption" color="text.secondary">方法 / 仪器{instruments.length > 1 ? '（请选择）' : ''}</Typography><Select fullWidth size="small" value={methodId} onChange={event => { const id = Number(event.target.value); setMethodId(id); const item = instruments.find(x => x.method_id === id); if (item) setMultiplier(item.multiplier); }} disabled={instruments.length === 1}>
            ...
            {instruments.map(item => <MenuItem key={item.method_id} value={item.method_id}>{item.instrument}{item.match_source === 'record' ? '（记录已填）' : '（检测类型匹配）'}</MenuItem>)}
          </Select></Box>}
          {!libraryAvailable && <Alert severity="warning" sx={{ mb: 1.5 }}>{preview?.reason || '未找到可用的检测方法，无法录入工作量。请先在管理后台维护项目与检测方法。'}</Alert>}
        </>}
        <Box sx={{ display: 'grid', gridTemplateColumns: 'repeat(2,minmax(0,1fr))', gap: 1 }}>
          <TextField label="本次录入数量" type="number" size="small" value={quantity} onChange={event => setQuantity(Number(event.target.value))} inputProps={{ min: 1, max: preview?.remaining_quantity ?? preview?.quantity }} helperText={`不得超过剩余数量 ${preview?.remaining_quantity ?? preview?.quantity}`} />
          <TextField label="金额倍率" type="number" size="small" value={multiplier} onChange={event => setMultiplier(Number(event.target.value))} inputProps={{ min: 0, step: 0.1 }} helperText="仅影响导出金额，不影响工作量" />
          <TextField label="计算工作量" size="small" value={(quantity * (preview?.coefficient ?? 1)).toFixed(2)} InputProps={{ readOnly: true }} helperText={`数量 × 系数 ${preview?.coefficient ?? 1}`} />
