...
const SortableSampleInfoColumnRow: React.FC<Props> = ({
  ...
  return (
    <TableRow ref={setNodeRef} hover sx={{ transform: CSS.Transform.toString(transform), transition, bgcolor: isDragging ? '#eef7f0' : undefined }}>
      ...
      <TableCell align="center"><Switch size="small" checked={column.is_required} onChange={(event) => onToggle(column, 'is_required', event.target.checked)} /></TableCell>
      ...
      <TableCell align="center"><Switch size="small" checked={column.show_in_export} onChange={(event) => onToggle(column, 'show_in_export', event.target.checked)} /></TableCell>
