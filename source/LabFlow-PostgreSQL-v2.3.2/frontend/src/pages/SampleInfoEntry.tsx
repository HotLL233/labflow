...
import {
  ...
  getServerTime, requireApiSuccess, type SampleInfoAttachmentPreviewStatus,
} from '../api/client';
...
const SampleInfoEntry: React.FC = () => {
  ...
  const formColumns = columns.filter((c: SampleInfoColumn) => c.show_in_form);
  // 列表列（show_in_list=true）
  const listColumns = columns.filter((c: SampleInfoColumn) => c.show_in_list);
  // 退回待修改记录沿用当前检测类型的实际表单列，避免编辑区写死字段。
  const editColumns = recordColumns
    .filter((c: SampleInfoColumn) => c.is_active && c.show_in_form)
  ...
  }).concat([{ key: '_action', header: '操作', fixed: 128, getValue: () => '' }])), [
