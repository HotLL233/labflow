...
fn workload_instruments(
    ...
    let mut stmt = conn.prepare(
        "SELECT DISTINCT m.id,i.id,
                ...
                m.multiplier,
                COALESCE(NULLIF(i.code,''),i.name) AS instrument_sort_name
         FROM projects p
         ...
         ORDER BY instrument_sort_name,m.id",
    )?;
...
async fn sample_workload_confirm(
    ...
    let workload = crate::models::record::RecordCreate {
        ...
        extra_fields: Some(
            serde_json::json!({"source_business_no": record.business_no, "source_notes": body.notes.unwrap_or_default(), "source_instrument_id": candidate.instrument_id}),
        ),
