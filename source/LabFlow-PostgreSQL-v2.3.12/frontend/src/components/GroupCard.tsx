...
const GroupCard: React.FC<GroupCardProps> = ({ group, onClick, themeColor }) => { 
  ...
  return (<Paper className="card-hover" onClick={onClick} sx={{ 
    ...
    {!!group.returned_sender_names && (
      <Box sx={{ mr: 3.5, mb: isMobile ? 0.75 : 1.25, px: 0.75, py: 0.4, borderRadius: '2px', bgcolor: '#c62828', color: '#fff', fontSize: '0.7rem', fontWeight: 700, lineHeight: 1.4, textAlign: 'left', overflowWrap: 'anywhere' }}>
        退回：{group.returned_sender_names}
      </Box>
    )}
