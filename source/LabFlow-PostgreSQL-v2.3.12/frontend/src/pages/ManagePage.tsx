  ...
  const saveCol = async () => {
    ...
    try {
      if (colEditItem) {
        ...
        if (r.code === 0) { showMessage('更新成功'); setColEditOpen(false); setColEditItem(null); loadSiColumns(siColTypeKey || undefined); }
        else showMessage(r.message, true);
      } else {
        ...
        if (r.code === 0) { showMessage('创建成功'); loadSiColumns(siColTypeKey || undefined); }
        else showMessage(r.message, true);
      }
    } catch (e: any) { showMessage(e.message || '操作失败', true); }
  };
