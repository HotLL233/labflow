...
const Wrap = ({ children, muted = false }: { children: React.ReactNode; muted?: boolean }) => (
  <Box sx={{ minWidth: 0, whiteSpace: 'normal', overflowWrap: 'break-word', wordBreak: 'normal', color: muted ? 'text.secondary' : 'text.primary', lineHeight: 1.45 }}>
    {children}
  </Box>
);
...
export default function SampleInfoRecordList(props: SampleInfoRecordListProps) {
  const {
    ...
    onEditFormChange, onStatusFlow, onWithdrawSample, onReturn, onConfirmReturn, currentUserId, isSystemAdmin, onLoadAttachments, onUploadAttachment, onDeleteAttachment,
  ...
  const RecordActions = ({ record }: { record: SampleInfoRecord }) => (
    <Box sx={{
      ...
      flexDirection: { xs: 'row', lg: 'column' }, flexWrap: 'wrap', width: '100%',
      '& .MuiButton-root': { width: { xs: 'auto', lg: '100%' }, minHeight: { xs: 44, lg: 32 }, whiteSpace: 'normal', lineHeight: 1.25 },
    }}>
      {!editable(record) && hasPermission('sample-info:edit-own') && (record.status === STATUS_WAIT_SAMPLE || record.status === STATUS_RETURN_DRAFT) && (isSystemAdmin || record.created_by_user_id === currentUserId || record.business_user_id === currentUserId) && <Button size="small" variant="outlined" startIcon={<EditIcon />} onClick={event => { event.stopPropagation(); onEdit(record); }} sx={{ borderRadius: R, whiteSpace: 'nowrap' }}>编辑</Button>}
      {record.status === STATUS_WAIT_SAMPLE && hasPermission('sample-info:collect') && <Button size="small" variant="contained" color="error" startIcon={<ScienceIcon />} onClick={event => { event.stopPropagation(); onStatusFlow(record.id, record.status); }} sx={{ borderRadius: R, whiteSpace: 'nowrap' }}>取样</Button>}
      {record.status === STATUS_WAIT_TEST && hasPermission('sample-info:withdraw') && <Button size="small" variant="outlined" color="warning" startIcon={<UndoIcon />} onClick={event => { event.stopPropagation(); onWithdrawSample(record.id); }} sx={{ borderRadius: R, whiteSpace: 'nowrap' }}>撤回取样</Button>}
      {record.status === STATUS_WAIT_TEST && hasPermission('sample-info:complete') && <Button size="small" variant="contained" color="success" startIcon={<CheckCircleIcon />} onClick={event => { event.stopPropagation(); onStatusFlow(record.id, record.status); }} sx={{ borderRadius: R, whiteSpace: 'nowrap' }}>完成检测</Button>}
      {record.status === STATUS_WAIT_SAMPLE && hasPermission('sample-info:return') && <Button size="small" variant="outlined" color="secondary" startIcon={<UndoIcon />} onClick={event => { event.stopPropagation(); onReturn(record); }} sx={{ borderRadius: R, whiteSpace: 'nowrap' }}>退回</Button>}
      {record.status === STATUS_RETURNED && hasPermission('sample-info:return-confirm') && (isSystemAdmin || record.created_by_user_id === currentUserId) && <Button size="small" variant="outlined" color="primary" startIcon={<UndoIcon />} onClick={event => { event.stopPropagation(); onConfirmReturn(record); }} sx={{ borderRadius: R, whiteSpace: 'normal' }}>确认退回</Button>}
    </Box>
  );

  const ColumnValue = ({ record, column }: { record: SampleInfoRecord; column: SampleInfoColumn }) => {
    if (column.field_key === 'status') return <Box sx={{ display: 'grid', gap: 0.35 }}><StatusChip status={record.status} />{record.return_reason && <Wrap muted>原因：{record.return_reason}</Wrap>}</Box>;
    if (column.data_type === 'attachment' || column.field_key === 'attachment_files') {
      return <AttachmentSummary record={record} attachmentsByRow={attachmentsByRow} attachments={attachments} loading={!!attachmentLoading[record.id]} onLoad={() => onLoadAttachments(record.id)} onUpload={file => onUploadAttachment(record.id, file)} onDelete={id => onDeleteAttachment(id, record.id)} onOpenAttachment={onOpenAttachment} hasPermission={hasPermission} currentUserId={currentUserId} isSystemAdmin={isSystemAdmin} />;
    }
    if (column.field_key === 'seq_no') {
      return <Wrap><Box sx={{ fontWeight: 700 }}>#{record.seq_no ?? '—'}</Box><Box sx={{ color: 'text.secondary', fontSize: '0.68rem', overflowWrap: 'anywhere' }}>{valueText(record.business_no)}</Box></Wrap>;
    }
    if (['submitted_at', 'detection_date', 'sampled_at'].includes(column.field_key)) {
      return <Wrap>{formatDate(String(getRecordValue(record, column.field_key) || ''))}</Wrap>;
    }
    return <Wrap>{valueText(getRecordValue(record, column.field_key))}</Wrap>;
  };
  ...
  return (
    <Paper elevation={0} sx={{ p: { xs: 1, sm: 1.5, lg: 2 }, borderRadius: R, border: '1px solid #e0e0e0' }}>
      ...
      {loading ? <Box sx={{ textAlign: 'center', py: 4 }}><CircularProgress size={32} /></Box> : (
        <>
          <TableContainer className={extraClass} sx={{ display: { xs: 'none', lg: 'block' }, width: '100%', overflowX: 'hidden', border: '1px solid #e5e5e5', borderRadius: R }}>
            <Table size="small" sx={{ width: '100%', tableLayout: 'fixed' }}>
              <colgroup>
                {columns.map(column => <col key={column.field_key} style={{ width: columnWidths[column.field_key]?.desktop }} />)}
              ...
              <TableBody>
                {records.length === 0 ? <TableRow><TableCell colSpan={Math.max(2, columns.length + 1)} align="center" sx={{ py: 4, color: 'text.secondary' }}>暂无登记记录</TableCell></TableRow> : records.map(record => (
                  <React.Fragment key={record.id}>
                    <TableRow hover sx={{ bgcolor: record.status === STATUS_DONE ? '#fbfbfb' : '#fff', verticalAlign: 'top', '& td': { px: 0.8, py: 1 } }}>
                      {columns.map(column => <TableCell key={column.field_key} sx={{ minWidth: 0, whiteSpace: 'normal', overflowWrap: 'break-word', wordBreak: 'normal' }}><ColumnValue record={record} column={column} /></TableCell>)}
                      <TableCell sx={{ minWidth: 0, verticalAlign: 'top', px: 0.75 }}><RecordActions record={record} /></TableCell>
                    </TableRow>
                    {editable(record) && <SampleInfoEditRow record={record} columnCount={columns.length} editColumns={editColumns} editForm={editForm} formatDate={formatDate} onEditFormChange={onEditFormChange} onCancel={onCancelEdit} onSave={onSaveEdit} divisionOptions={divisionOptions} />}
                  </React.Fragment>
                ))}
              </TableBody>
            </Table>
                ...
                <CardContent sx={{ p: 1.25, '&:last-child': { pb: 1.25 } }}>
                  ...
                  {editable(record) ? <Box onClick={event => event.stopPropagation()} onMouseDown={event => event.stopPropagation()} sx={{ display: 'grid', gap: 1 }}>
                    ...
                    <Box sx={{ display: 'grid', gridTemplateColumns: 'repeat(2,minmax(0,1fr))', gap: 1, mb: 1 }}>
                      {columns.filter(column => column.field_key !== 'status' && column.data_type !== 'attachment' && column.field_key !== 'attachment_files').map(column => <Wrap key={column.field_key}><Label>{column.label}</Label>{column.field_key === 'submitted_at' ? formatDate(record.submitted_at) : valueText(getRecordValue(record, column.field_key))}</Wrap>)}
                    </Box>
                    ...
                    <Box sx={{ mt: { xs: 1.75, lg: 1 } }}><RecordActions record={record} /></Box>
                  </>}
                </CardContent>
              </Card>
            ))}
