...
export default function SampleInfoRecordList(props: SampleInfoRecordListProps) {
  ...
  const actionButtonSx = (fieldKey: string) => {
    ...
    return {
      borderRadius: R,
      width: { xs: 'auto', lg: width ? `${width}px` : 'auto' },
      minWidth: { xs: 0, lg: width ? `${width}px` : undefined },
      flex: { xs: '0 0 auto', lg: width ? `0 0 ${width}px` : '0 0 auto' },
                ...
                {records.length === 0 ? <TableRow><TableCell colSpan={Math.max(2, dataColumns.length + 1)} align="center" sx={{ py: 4, color: 'text.secondary' }}>暂无登记记录</TableCell></TableRow> : records.map(record => (
                  <React.Fragment key={record.id}>
                    <TableRow hover sx={{ bgcolor: record.status === STATUS_DONE ? '#fbfbfb' : '#fff', verticalAlign: 'top', '& td': { px: 0.8, py: 1 } }}>
                      {dataColumns.map(column => <TableCell key={column.field_key} sx={{ ...recordCellSx, width: columnWidths[column.field_key]?.desktop, overflow: 'hidden' }}><ColumnValue record={record} column={column} /></TableCell>)}
                      <TableCell sx={{ ...recordCellSx, width: columnWidths._action?.desktop, px: 0.5, textAlign: 'center', overflow: 'hidden' }}><AllActions record={record} /></TableCell>
                    </TableRow>
