    ...
    <DialogContent>
      {preview?.recorded ? <Alert severity="info" sx={{ mb: 1.5 }}>本次取样已录入工作量。</Alert> : <>
        <Box sx={{ display: 'grid', gridTemplateColumns: 'repeat(2,minmax(0,1fr))', gap: 1, mt: 0.5, mb: 1.5 }}>
          {[['当前录入人', preview?.operator_username], ['部门', preview?.department], ['实验室', preview?.laboratory], ['项目', preview?.project], ['方法/类型', preview?.method || preview?.detection_type]].map(([label, value]) =>
