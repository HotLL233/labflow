...
const ManagePage: React.FC = () => {
  ...
  const siRecordWidths = useMemo(() => getAdaptiveColumnWidths(siRecords, [
    ...visibleSiRecordColumns.map(column => ({
      ...
      header: column.label,
      min: column.data_type === 'number' ? 58 : 68,
      max: ['notes', 'main_components'].includes(column.field_key) ? 220 : 150,
      getValue: (record: SampleInfoRecord) => getSiRecordValue(record, column.field_key),
    })),
    { key: 'actions', header: '操作', fixed: 76, getValue: () => '' },
  ]), [siRecords, visibleSiRecordColumns, getSiRecordValue]);
  ...
  const [requiredTypesSelected, setRequiredTypesSelected] = useState<string[]>([]);
  useEffect(() => {
  ...
  const saveCol = async () => {
    ...
    try {
      if (colEditItem) {
        ...
        if (r.code === 0) { showMessage('更新成功'); setColEditOpen(false); setColEditItem(null); loadSiColumns(); }
        else showMessage(r.message, true);
      } else {
        ...
        if (r.code === 0) { showMessage('创建成功'); loadSiColumns(); }
        else showMessage(r.message, true);
      }
    } catch (e: any) { showMessage(e.message || '操作失败', true); }
  };
  ...
  const saveSampleColumnToggle = async (
    column: SampleInfoColumn,
    key: 'is_required' | 'is_active' | 'show_in_form' | 'show_in_list' | 'show_in_export',
    value: boolean,
  ) => {
    try {
      if (key === 'is_active' || !siColTypeKey) {
        const response = await updateSampleInfoColumn(column.id, { [key]: value });
        if (response.code !== 0) throw new Error(response.message || '字段配置更新失败');
      } else {
        const next = { ...column, [key]: value } as SampleInfoColumn;
        if (key === 'show_in_form' && !value) next.is_required = false;
        const response = await updateSampleInfoColumnVisibility({ type_key: siColTypeKey, items: [{
          column_id: column.id,
          is_visible: column.is_visible_in_type !== false,
          is_required: next.is_required,
          show_in_form: next.show_in_form,
          show_in_list: next.show_in_list,
          show_in_export: next.show_in_export,
          sort_order: next.sort_order,
        }] });
        if (response.code !== 0) throw new Error(response.message || '字段配置更新失败');
      }
      setSiColumns(current => current.map(item => item.id === column.id ? { ...item, [key]: value, ...(key === 'show_in_form' && !value ? { is_required: false } : {}) } : item));
    } catch (error: any) { showMessage(error?.message || '字段配置更新失败', true); }
  };
  const saveSampleColumnTypeVisibility = async (column: SampleInfoColumn, value: boolean) => {
    if (!siColTypeKey) return;
    try {
      const response = await updateSampleInfoColumnVisibility({ type_key: siColTypeKey, items: [{
        column_id: column.id,
        is_visible: value,
        is_required: value && column.show_in_form && column.is_required,
        show_in_form: value && column.show_in_form,
        show_in_list: value && column.show_in_list,
        show_in_export: value && column.show_in_export,
        sort_order: column.sort_order,
      }] });
      if (response.code !== 0) throw new Error(response.message || '字段显示范围更新失败');
      setSiColumns(current => current.map(item => item.id === column.id ? { ...item, is_visible_in_type: value } : item));
    } catch (error: any) { showMessage(error?.message || '字段显示范围更新失败', true); }
  };
    ...
    {showSectionContent && activeTab === 'sampleinfo' && <Box>
      ...
      <Paper elevation={0} sx={{ p: 2, mb: 3, borderRadius: BORDER_RADIUS, border: '1px solid rgba(0,0,0,0.08)' }}>
        ...
        <Typography variant="caption" color="text.secondary" sx={{ mb: 1, display: 'block' }}>
          当前表格只编辑所选检测类型。表单、列表、导出和必填互相独立；关闭表单后该类型不再执行必填校验。
        </Typography>
        {siColumns.length === 0 ? (
          ...
          <DndContext sensors={sampleColumnSensors} collisionDetection={closestCenter} onDragEnd={reorderSampleColumnsByDrag}>
            <Table size="small" sx={{ minWidth: 1220 }}>
              ...
              <SortableContext items={siColumns.map(column => column.id)} strategy={verticalListSortingStrategy}>
                <TableBody>{siColumns.map((column, index) => <SortableSampleInfoColumnRow key={column.id} column={column} index={index} sampleTypes={sampleTypes}
                  onEdit={editCol} onDelete={item => delCol(item.id)} onToggle={saveSampleColumnToggle}
                  onTypes={(_event, item) => { void saveSampleColumnTypeVisibility(item, item.is_visible_in_type === false); }} />)}</TableBody>
              </SortableContext>
            </Table>
          </DndContext>
          </TableContainer>
      ...
      <Popover open={!!visibleTypesAnchorEl} anchorEl={visibleTypesAnchorEl}
        ...
        slotProps={{ paper: { sx: { borderRadius: BORDER_RADIUS, p: 1.5, minWidth: 200 } } }}>
        ...
        <List dense sx={{ maxHeight: 240, overflow: 'auto', py: 0 }}>
          {sampleTypes.map(st => (
            <ListItem key={st.type_key} disableGutters sx={{ py: 0 }}>
              ...
              <FormControlLabel sx={{ mr: 0 }} label="必填" control={<Checkbox size="small" checked={requiredTypesSelected.includes(st.type_key)} disabled={!visibleTypesSelected.includes(st.type_key)} onChange={() => setRequiredTypesSelected(prev => prev.includes(st.type_key) ? prev.filter(t => t !== st.type_key) : [...prev, st.type_key])} />} />
            </ListItem>
          ))}
        </List>
        <Box sx={{ display: 'flex', gap: 0.5, justifyContent: 'flex-end', mt: 1 }}>
          <Button size="small" onClick={() => setVisibleTypesAnchorEl(null)} sx={{ borderRadius: BORDER_RADIUS, fontSize: '0.75rem' }}>取消</Button>
          <Button size="small" variant="contained" sx={{ borderRadius: BORDER_RADIUS, fontSize: '0.75rem', bgcolor: '#2e7d32' }}
            onClick={async () => {
              if (visibleTypesColId === null) return;
              try {
                const rules = sampleTypes.map(st => ({ type_key: st.type_key, is_visible: visibleTypesSelected.includes(st.type_key), is_required: requiredTypesSelected.includes(st.type_key) }));
                const r = await updateSampleInfoColumnTypes(visibleTypesColId, rules);
                if (r.code === 0) {
                  setSiColumns(prev => prev.map(c => c.id === visibleTypesColId ? { ...c, visible_types: visibleTypesSelected, required_types: requiredTypesSelected.filter(t => visibleTypesSelected.includes(t)) } : c));
                  setVisibleTypesAnchorEl(null);
                  showMessage('显示类型更新成功');
                } else {
                  showMessage(r.message || '更新失败', true);
                }
              } catch (e: any) {
                showMessage(e.message || '更新失败', true);
              }
            }}>保存</Button>
        </Box>
      </Popover>
          ...
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2, mt: 1 }}>
            ...
            <Box sx={{ display: 'flex', gap: 2 }}>
              <TextField label="宽度(px)" type="number" size="small" value={colForm.width}
                ...
                }} inputProps={{ min: 48, max: 500 }} helperText="记录列表按宽度比例铺满页面，长内容在单元格内滚动" sx={{ width: 190 }} />
             ...
             <FormControlLabel control={<Switch checked={colForm.is_required} disabled={colForm.data_type === 'action'} onChange={e => setColForm(p => ({ ...p, is_required: e.target.checked }))} />} label="必填" />
             <Box sx={{ display: 'flex', gap: 2 }}>
