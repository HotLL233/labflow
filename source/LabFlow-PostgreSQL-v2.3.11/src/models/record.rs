use serde::{Deserialize, Serialize};

#[derive(Debug, Serialize)]
pub struct RecordResponse {
    pub id: i64,
    pub business_no: String,
    pub project_id: i64,
    pub method_id: Option<i64>,
    pub project_name: String,
    pub group_name: String,
    pub user_name: String,
    pub quantity: i32,
    pub recorded_at: String,
    pub created_at: String,
    pub deleted_at: Option<String>,
    pub method_name: Option<String>,
    pub method_type: Option<String>,
    pub instrument_code: String,
    pub instrument_type: String,
    pub multiplier: f64,
    pub high_item: Option<String>,
    pub coefficient_snapshot: f64,
    pub subject_user_id: Option<i64>,
    pub created_by_user_id: Option<i64>,
    pub project_division_id: Option<i64>,
    pub execution_division_id: Option<i64>,
    pub execution_group_id: Option<i64>,
    pub detection_division_id: Option<i64>,
    pub sending_division_id: Option<i64>,
    pub sending_group_id: Option<i64>,
}

#[derive(Debug, Deserialize)]
pub struct RecordCreate {
    pub project_id: i64,
    pub method_id: Option<i64>,
    pub user_name: String,
    #[serde(default)]
    pub sender_user_id: Option<i64>,
    pub quantity: i32,
    pub recorded_at: String,
    pub group_id: Option<i64>,
    pub multiplier: Option<f64>,
    #[serde(default)]
    pub high_item: Option<String>,
    #[serde(default)]
    pub division_id: Option<i64>,
    #[serde(default)]
    pub extra_fields: Option<serde_json::Value>,
    /// Set only for work records created from a sampled business record.
    #[serde(default)]
    pub source_type: Option<String>,
    #[serde(default)]
    pub source_record_id: Option<i64>,
}

#[derive(Debug, Deserialize)]
pub struct RecordUpdate {
    pub user_name: Option<String>,
    /// Actual sender identity. The display name is kept as a snapshot in the
    /// record while this ID drives access to returned records.
    #[serde(default)]
    pub sender_user_id: Option<i64>,
    pub quantity: Option<i32>,
    pub recorded_at: Option<String>,
    pub multiplier: Option<f64>,
    // v0.4.34: 行内编辑扩展字段
    pub project_id: Option<i64>,
    pub method_id: Option<i64>,
    pub group_id: Option<i64>,
    pub division_id: Option<i64>,
    pub batch_no: Option<String>,
    pub notes: Option<String>,
    // v0.4.67: 高项自定义编辑
    pub high_item: Option<String>,
    #[serde(default)]
    pub extra_fields: Option<serde_json::Value>,
    #[serde(skip_deserializing, default)]
    pub source_type: Option<String>,
    #[serde(skip_deserializing, default)]
    pub source_record_id: Option<i64>,
}
