...
const PersonnelChangePage: React.FC = () => {
  ...
  return <Box>
    ...
    <Paper variant="outlined" sx={{ p: 2, mb: 2 }}>
      ...
      <TableContainer><Table size="small"><TableHead><TableRow>{['通知编号', '变动类型', '实验室', '人员', '项目/方法', '生效日期', '状态', '操作'].map(title => <TableCell key={title} sx={{ fontWeight: 800 }}>{title}</TableCell>)}</TableRow></TableHead><TableBody>
    ...
    {canManageBaseline && <Paper variant="outlined" sx={{ p: 2 }}>
      ...
      <TableContainer><Table size="small"><TableHead><TableRow>{['实验室', '实验室负责人', '实验人员', '项目代号', '检测方法'].map(title => <TableCell key={title} sx={{ fontWeight: 800 }}>{title}</TableCell>)}</TableRow></TableHead><TableBody>
