...
const SortableColumnRow: React.FC<{
  ...
  return <TableRow ref={setNodeRef} hover sx={style}>
    ...
    <TableCell align="center"><Checkbox size="small" checked={column.is_required} disabled /></TableCell>
    <TableCell align="center"><Switch size="small" checked={column.is_active} disabled /></TableCell>
    <TableCell align="center"><Checkbox size="small" checked={column.show_in_form} disabled /></TableCell>
    <TableCell align="center"><Checkbox size="small" checked={column.show_in_list} disabled /></TableCell>
    <TableCell align="center"><Checkbox size="small" checked={column.show_in_export} disabled /></TableCell>
    ...
    <Box>
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 1, flexWrap: 'wrap', mb: 1.5 }}>
        <Box>
          ...
          <Typography variant="caption" color="text.secondary">启用字段 {activeCount} 个，按住排序图标可拖拽调整顺序</Typography>
        </Box>
        <Button variant="contained" size="small" startIcon={<AddIcon />} onClick={openCreate} sx={{ borderRadius: R }}>新增字段</Button>
      </Box>
      <Paper elevation={0} sx={{ borderRadius: R, border: '1px solid rgba(0,0,0,0.1)', overflowX: 'auto' }}>
        <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={reorderByDrag}><Table size="small" sx={{ minWidth: 930 }}>
          <TableHead>
            <TableRow sx={{ bgcolor: '#f5f5f5' }}>
              <TableCell sx={{ fontWeight: 700, width: 82 }}>排序</TableCell>
              <TableCell sx={{ fontWeight: 700 }}>字段名称</TableCell>
              <TableCell sx={{ fontWeight: 700 }}>输入方式</TableCell>
              <TableCell sx={{ fontWeight: 700, textAlign: 'center' }}>必填</TableCell>
              <TableCell sx={{ fontWeight: 700, textAlign: 'center' }}>启用</TableCell>
              <TableCell sx={{ fontWeight: 700, textAlign: 'center' }}>表单</TableCell>
              <TableCell sx={{ fontWeight: 700, textAlign: 'center' }}>列表</TableCell>
              <TableCell sx={{ fontWeight: 700, textAlign: 'center' }}>导出</TableCell>
              <TableCell sx={{ fontWeight: 700, textAlign: 'center' }}>宽度</TableCell>
              <TableCell sx={{ fontWeight: 700, textAlign: 'center', width: 90 }}>操作</TableCell>
            </TableRow>
          </TableHead>
          <SortableContext items={cols.map(column => column.id)} strategy={verticalListSortingStrategy}><TableBody>
            ...
            {cols.length === 0 && <TableRow><TableCell colSpan={10} align="center" sx={{ py: 4, color: 'text.secondary' }}>暂无字段配置</TableCell></TableRow>}
