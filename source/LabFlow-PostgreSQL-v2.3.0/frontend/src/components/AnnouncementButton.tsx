...
export default function AnnouncementButton() {
  ...
  return <><Button startIcon={<NotificationsActiveIcon />} onClick={() => setOpen(true)} disabled={!items.length} sx={{ color: '#475569', fontWeight: 500 }}>公告</Button><Dialog open={open} onClose={() => setOpen(false)} maxWidth="sm" fullWidth><DialogTitle>系统公告</DialogTitle><DialogContent dividers><Stack spacing={1.5}>{items.map((a, i) => <Alert key={a.id || i} severity={a.level || 'info'}><strong>{a.title}</strong>{a.content ? `：${a.content}` : ''}</Alert>)}</Stack></DialogContent><DialogActions><Button onClick={() => setOpen(false)}>关闭</Button></DialogActions></Dialog></>;
}
