...
async fn sample_workload_preview(
    ...
    Ok(Json(ApiResponse::ok(SampleWorkloadPreview {
        ...
        remaining_quantity: (record.quantity as i32 - crate::repo::record_repo::source_quantity(&pool, "rd_sample", id)?).max(0),
        coefficient: method.coefficient,
        multiplier: method.multiplier,
        recorded: crate::repo::record_repo::source_fully_recorded(&pool, "rd_sample", id, record.quantity as i32)?,
    })))
}
