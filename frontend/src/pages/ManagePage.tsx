...
type TV = 'projects' | 'groups' | 'methods' | 'divisions' | 'master-import' | 'data-governance' | 'trash' | 'audit' | 'backup' | 'help' | 'sampleinfo' | 'users' | 'roles' | 'layouts' | 'theme' | 'home' | 'stats' | 'forms' | 'exports' | 'sessions' | 'log-maintenance';

const ROUTABLE_TABS: TV[] = ['projects', 'groups', 'divisions', 'master-import', 'methods', 'trash', 'audit', 'backup', 'help', 'sampleinfo', 'users', 'roles', 'layouts', 'forms', 'exports', 'stats', 'sessions', 'log-maintenance'];
...
const AL: Record<string, string> = { create: '创建', update: '更新', delete: '删除', restore: '恢复', import: '导入', sample: '取样', status_change: '状态流转', legacy_import: '历史基线' };
const AC: Record<string, 'success' | 'error' | 'info' | 'warning' | 'default'> = { create: 'success', update: 'info', delete: 'error', restore: 'warning', import: 'warning', sample: 'info', status_change: 'info', legacy_import: 'default' };
const TL: Record<string, string> = { work_records: '分析检测记录', rd_work_records: '研发送样记录', sample_info_records: '样品信息登记', projects: '项目', project_groups: '实验室', samples: '送样记录(已退役)' };
const ML: Record<string, string> = { work: '分析检测', rd: '研发送样', sample_info: '样品信息', shared: '主数据' };
...
const ManagePage: React.FC = () => {
  ...
  const [ca, setCa] = useState<(reason?: string) => Promise<void>>(() => async () => { setCo(false); });
  ...
  const openRecycleConfirm = useCallback((
    ...
    setCa(() => async (reason?: string) => { await action(reason); });
    setCo(true);
  ...
  const routeTb = useMemo<TV | null>(() => {
    if (!section) return null;
    return ROUTABLE_TABS.includes(section as TV) ? (section as TV) : null;
  }, [section]);
  ...
  const defaultTC: { key: TV; label: string; perm?: string; icon: React.ReactNode; desc: string }[] = [
    ...
    { key: 'master-import', label: '主数据一键导入', perm: 'manage:master-import', icon: <CloudUploadIcon />, desc: '按模板批量导入主数据及关联关系' },
  ...
  const visibleCards = useMemo(
    () => tc.filter(c => user?.is_admin || !c.perm || hasPermission(user?.permissions || [], c.perm)),
    [tc, user]
  );
  const activeCard = useMemo(
    () => defaultTC.find(c => c.key === tb) || tc.find(c => c.key === tb) || null,
    [defaultTC, tb, tc]
  );
  const enabledNavKeys = useMemo(() => {
    const keys = new Set<ManageNavKey>(tc.map(card => card.key as ManageNavKey));
    keys.add('sessions');
    keys.add('stats');
  ...
  const loadTrash = useCallback(async (page = 1) => {
    ...
    try {
      ...
      if (response.code === 0 && response.data) {
        ...
        sm(response.message || '加载回收站失败', true);
      }
    } catch (error: any) {
      sm(error?.message || '加载回收站失败', true);
    } finally {
  ...
  const handleRestoreTrash = async (entry: TrashEntry) => {
    try {
      ...
      if (response.code === 0) {
        sm(`已恢复：${entry.display_name}`);
        await loadTrash(trashPage);
        if (entry.table_name === 'backup_files') await loadBk();
      } else sm(response.message, true);
    } catch (error: any) { sm(error?.message || '恢复失败', true); }
  };
  ...
  const handlePurgeTrash = async () => {
    ...
    try {
      ...
      if (response.code === 0) {
        sm(`已永久清理：${trashPurgeTarget.display_name}`);
        setTrashPurgeTarget(null);
        await loadTrash(trashPage);
      } else sm(response.message, true);
    } catch (error: any) { sm(error?.message || '永久清理失败', true); }
  ...
  const trashRecordWidths = useMemo(() => getAdaptiveColumnWidths(trashItems, [
    ...
    { key: 'module', header: '模块', min: 72, max: 96, getValue: r => ML[r.module] || r.module },
  ...
  const auditLogWidths = useMemo(() => getAdaptiveColumnWidths(al, [
    ...
    { key: 'module', header: '模块', min: 72, max: 96, getValue: r => ML[r.module] || r.module },
    { key: 'action', header: '操作类型', min: 74, max: 110, getValue: r => AL[r.action] || r.action },
    { key: 'business_no', header: '业务编号', min: 150, max: 220, getValue: r => r.business_no },
    { key: 'user_name', header: '操作人', min: 62, max: 110, getValue: r => r.user_name },
  ...
  const siRecordWidths = useMemo(() => getAdaptiveColumnWidths(siRecords, [
    ...
    { key: 'lab_name', header: '实验室', min: 72, max: 130, getValue: r => r.lab_name },
    { key: 'project_name', header: '项目', min: 78, max: 150, getValue: r => r.project_name },
    { key: 'detection_type', header: '检测类型', min: 78, max: 130, getValue: r => r.detection_type },
    { key: 'status', header: '状态', fixed: 76, getValue: r => r.status },
    { key: 'detection_date', header: '检测时间', fixed: 112, getValue: r => r.detection_date },
    { key: 'main_components', header: '主要成分', min: 130, max: 240, getValue: r => r.main_components },
    { key: 'actions', header: '操作', fixed: 76, getValue: () => '' },
  ]), [siRecords]);

  // v0.4.71: 显示类型编辑
  const [sampleTypes, setSampleTypes] = useState<{ type_key: string; label: string }[]>([]);
  const [visibleTypesAnchorEl, setVisibleTypesAnchorEl] = useState<HTMLElement | null>(null);
  const [visibleTypesColId, setVisibleTypesColId] = useState<number | null>(null);
  const [visibleTypesSelected, setVisibleTypesSelected] = useState<string[]>([]);
  useEffect(() => {
    getSampleInfoTypesAll().then(r => { if (r.code === 0 && r.data) setSampleTypes(r.data); }).catch(() => {});
  }, []);
  ...
  const loadStatsRoles = useCallback(async () => {
    try {
      ...
      } else sm(response.message || '加载统计权限失败', true);
    } catch (error: any) { sm(error?.message || '加载统计权限失败', true); }
  }, [sm]);
  ...
  const saveStatsPermissions = async () => {
    ...
    } catch (error: any) { sm(error?.message || '统计权限保存失败', true); }
  ...
  const filteredUsers = useMemo(() => {
    ...
    return users.filter((item) => {
      const textMatch = !query || `${item.id} ${item.username} ${item.division_name || ''} ${item.group_name || ''} ${(item.affiliation_groups || []).join(' ')} ${(item.role_names || []).join(' ')}`.toLowerCase().includes(query);
      const divisionMatch = !userDivisionFilter || String(item.division_id || '') === userDivisionFilter;
      const affiliationMatch = !userAffiliationFilter || (item.affiliation_groups || []).includes(userAffiliationFilter);
      const groupMatch = !userGroupFilter || String(item.group_id || '') === userGroupFilter;
  ...
  const handleSaveUser = async () => {
    ...
    try {
      if (userEditItem) {
        const body: UserUpdate = { username: userForm.username, division_id: userForm.division_id, group_id: userForm.group_id, role_ids: userForm.role_ids, is_active: userForm.is_active };
        if (userForm.password.trim()) body.password = userForm.password;
        const result = await updateUser(userEditItem.id, body);
        if (result.code !== 0) {
          sm(result.message || '用户更新失败', true);
          return;
        }
        sm('用户更新成功');
      } else {
        await createUser({ username: userForm.username, password: userForm.password, division_id: userForm.division_id, group_id: userForm.group_id, role_ids: userForm.role_ids });
        sm('用户创建成功');
      }
  ...
  const handleDeleteUser = async (id: number) => {
    ...
    openRecycleConfirm('users', id, target?.username || `用户#${id}`, async (reason) => {
      try { await deleteUser(id, reason); sm('用户已删除'); loadUsers(); setCo(false); }
  ...
  const openSiEdit = (rec: SampleInfoRecord) => {
    ...
    setSiEditForm({
      ...
      project_name: rec.project_name, main_components: rec.main_components, notes: rec.notes,
    });
  };
  ...
  const saveSiType = async () => {
    ...
    try {
      if (siTypeEdit) {
        ...
        if (r.code === 0) { sm('更新成功'); setSiTypeEdit(null); setSiTypeNewOpen(false); setSiTypeForm({ type_key: '', label: '', description: '', color: '#2e7d32', sort_order: 0, is_active: 1 }); loadSiTypes(); }
        else sm(r.message, true);
      } else {
        ...
        if (r.code === 0) { sm('创建成功'); setSiTypeNewOpen(false); setSiTypeForm({ type_key: '', label: '', description: '', color: '#2e7d32', sort_order: 0, is_active: 1 }); loadSiTypes(); }
        else sm(r.message, true);
      }
  ...
  useEffect(() => { setLd(true); Promise.all([lg(), lp(), lm(), li(), lmt()]).finally(() => setLd(false)); }, [lg, lp, lm, li, lmt]);
  useEffect(() => { if (tb === 'audit') la(1); if (tb === 'backup') loadBk(); if (tb === 'methods') { lmt(); lm(); li(); } if (tb === 'trash') loadTrash(1); if (tb === 'stats') loadStatsRoles(); if (tb === 'help') { loadHelpDocs(); loadHelpArticles(); } if (tb === 'users') { userList().then(r => { if (r.code === 0 && r.data) setUsers(r.data); }).catch(() => {}); getRoles().then(r => { if (r.code === 0 && r.data) setRoles(r.data); }).catch(() => {}); } }, [tb, la, lmt, lm, li, loadUsers, loadTrash, loadStatsRoles]);
  ...
  const handleProjectStatusChange = async (project: Project, status: 'ongoing' | 'archived') => {
    ...
    if (r.code === 0) { sm(status === 'archived' ? '项目已归档' : '项目已重新启用'); lp(); }
    else sm(r.message, true);
  };
  ...
  const handleSaveMethod = async (method: Method) => {
    if (!method.name.trim()) { sm('请输入方法名称', true); return; }
    if (method.name.includes('@')) { sm('方法名称不能包含 @ 仪器识别字符', true); return; }
    if (!method.instrument_id) { sm('请选择对应仪器', true); return; }
    const body: any = {
      name: method.name,
  ...
  const handleSaveInstrument = async () => {
    if (!instrumentEdit.code.trim()) { sm('请输入仪器编号', true); return; }
    if (!instrumentEdit.instrument_type.trim()) { sm('请输入仪器类型', true); return; }
    const body = {
  ...
  if (section && !routeTb) return <Navigate to="/404" replace />;
  if (routeTb && activeCard?.perm && !user?.is_admin && !hasPermission(user?.permissions || [], activeCard.perm)) {
    return <Navigate to="/manage" replace />;
  }
  if (ld && !isManageHome) return <Box sx={{ display: 'flex', justifyContent: 'center', mt: 8 }}><CircularProgress /></Box>;

  return (
    <Box sx={{ display: { xs: 'block', md: 'flex' }, alignItems: 'flex-start', gap: 2 }}>
      <ManageNav activeKey={isManageHome ? '' : (tb as ManageNavKey)} enabledKeys={enabledNavKeys} />
      <Box sx={{ flex: 1, minWidth: 0 }}>
        ...
        {isManageHome ? (
          ...
          <Box sx={{ position: 'sticky', top: { xs: 56, md: 64 }, zIndex: 5, bgcolor: 'background.default', py: 1, display: 'flex', alignItems: 'center', gap: 1.5, mb: 2, flexWrap: 'wrap', borderBottom: '1px solid rgba(0,0,0,0.08)' }}>
            <Button variant="outlined" startIcon={<ArrowBackIcon />} onClick={() => navigate('/manage')} sx={{ borderRadius: R }}>
              返回管理工作区
            </Button>
            <Box>
    ...
    {showSectionContent && tb === 'projects' && <Box>
      <Box sx={{ position: 'sticky', top: { xs: 112, md: 130 }, zIndex: 3, bgcolor: 'background.default', py: 1, display: 'flex', gap: 1, mb: 2, flexWrap: 'wrap', justifyContent: 'flex-end' }}>
        <TextField size="small" placeholder="搜索项目名称、高项" value={projectSearch} onChange={e => setProjectSearch(e.target.value)} sx={{ minWidth: { xs: '100%', sm: 220 }, '& .MuiOutlinedInput-root': { borderRadius: R } }} />
        <Button variant="contained" startIcon={<AddIcon />} onClick={() => {
          setProjectEditItem({ id: 0, name: '', full_name: '', notes: '', sort_order: 0, is_active: true, project_status: 'ongoing', lab_ids: [], method_ids: [], lab_names: [], method_names: [], type_names: [], created_at: '', updated_at: '' } as Project);
    ...
    {/* ── 2. 实验室管理 ── */}
    {showSectionContent && tb === 'groups' && <Box>
      <Box sx={{ position: 'sticky', top: { xs: 112, md: 130 }, zIndex: 3, bgcolor: 'background.default', py: 1, display: 'flex', justifyContent: 'flex-end', mb: 2, gap: 1, flexWrap: 'wrap' }}>
        <TextField size="small" placeholder="搜索实验室" value={groupSearch} onChange={e => setGroupSearch(e.target.value)} sx={{ minWidth: { xs: '100%', sm: 200 }, '& .MuiOutlinedInput-root': { borderRadius: R } }} />
        <Button variant="contained" startIcon={<AddIcon />} onClick={() => {
      ...
      {gs.length === 0 ? <Typography color="text.secondary" textAlign="center" sx={{ py: 4 }}>暂无实验室分组</Typography>
        : <Paper variant="outlined" elevation={0} sx={{ borderRadius: R, overflow: 'hidden' }}>
          {gs.filter(g => g.name.toLowerCase().includes(groupSearch.trim().toLowerCase())).map((g, index, filtered) => (
            <Box key={g.id} sx={{ display: 'grid', gridTemplateColumns: { xs: 'minmax(0,1fr) auto', md: 'minmax(0,1fr) 120px 100px' }, alignItems: 'center', gap: 1.5, px: { xs: 1.25, md: 2 }, py: 1.1, borderBottom: index === filtered.length - 1 ? 'none' : '1px solid #edf0f2' }}>
              ...
              <Box sx={{ display: 'flex', justifyContent: 'flex-end', gap: 0.25 }}>
                ...
                <IconButton size="small" color="error" onClick={() => openRecycleConfirm('project_groups', g.id, g.name, async (reason) => { const r = await deleteGroup(g.id, reason); if (r.code === 0) { sm('删除成功'); lg(); } else sm(r.message, true); setCo(false); })} aria-label={`删除${g.name}`}><DeleteIcon fontSize="small" /></IconButton>
              </Box>
            </Box>
          ))}
        </Paper>}
    ...
    {/* ── 2.5 事业部管理 (v0.4.24) ── */}
    {showSectionContent && tb === 'divisions' && <Box>
      <Box sx={{ position: 'sticky', top: { xs: 112, md: 130 }, zIndex: 3, bgcolor: 'background.default', py: 1, display: 'flex', justifyContent: 'flex-end', mb: 2, gap: 1, flexWrap: 'wrap' }}>
        <TextField size="small" placeholder="搜索部门" value={divisionSearch} onChange={e => setDivisionSearch(e.target.value)} sx={{ minWidth: { xs: '100%', sm: 200 }, '& .MuiOutlinedInput-root': { borderRadius: R } }} />
        <Button variant="contained" startIcon={<AddIcon />} onClick={() => {
            ...
            <TableBody>
              {divs.filter(d => d.name.toLowerCase().includes(divisionSearch.trim().toLowerCase())).map(d => <TableRow key={d.id} hover>
                ...
                <TableCell align="right">
                  ...
                  <IconButton size="small" color="error" onClick={() => openRecycleConfirm('divisions', d.id, d.name, async (reason) => { const r = await deleteDivision(d.id, reason); if (r.code === 0) { sm('删除成功'); ldiv(); lg(); } else sm(r.message, true); setCo(false); })}><DeleteIcon fontSize="small" /></IconButton>
                </TableCell>
              </TableRow>)}
            </TableBody>
          </Table>
    ...
    {showSectionContent && tb === 'methods' && <Box>
      <Box sx={{ position: 'sticky', top: { xs: 112, md: 130 }, zIndex: 3, bgcolor: 'background.default', py: 1, display: 'flex', flexDirection: { xs: 'column', md: 'row' }, justifyContent: 'space-between', alignItems: { xs: 'stretch', md: 'center' }, gap: 1.25, mb: 2 }}>
        <Box>
        ...
        <Box sx={{ display: { xs: 'grid', sm: 'flex' }, gridTemplateColumns: { xs: 'repeat(2,minmax(0,1fr))' }, flexWrap: 'wrap', gap: 1, width: { xs: '100%', md: 'auto' }, '& > *': { minWidth: '0 !important' }, '& .MuiButton-root': { width: { xs: '100%', sm: 'auto' } } }}>
          <TextField size="small" placeholder="搜索方法或仪器" value={methodSearch} onChange={e => setMethodSearch(e.target.value)} sx={{ minWidth: { xs: '100%', sm: 180 }, '& .MuiOutlinedInput-root': { borderRadius: R } }} />
          <Button variant="outlined" startIcon={<VisibilityIcon />} size="small"
          ...
          <Button variant="outlined" startIcon={<CloudUploadIcon />} size="small"
            ...
            sx={{ borderRadius: R, width: isMobile ? '100%' : undefined }}>
            主数据导入
          </Button>
          <Button variant="contained" startIcon={<AddIcon />} onClick={() => {
            setMethodEditItem({ id: 0, method_code: '', name: '', full_name: '', instrument_id: instruments.find(i => i.is_active)?.id || 0, instrument_code: '', instrument_name: '', instrument_type: '', coefficient: 1.0, multiplier: 1.0, amount: 0, notes: '', type_ids: [] as number[], type_names: [] as string[], is_active: true, created_at: '', updated_at: '' } as Method);
      ...
      {ml.filter(m => (!methodFilter || m.type_names.includes(methodFilter)) && `${m.name} ${m.full_name || ''} ${m.instrument_code || ''} ${m.instrument_name || ''}`.toLowerCase().includes(methodSearch.trim().toLowerCase())).length === 0 ? <Typography color="text.secondary" textAlign="center" sx={{ py: 4 }}>暂无检测方法数据，请先导入方法或手动创建</Typography>
        : <Paper variant="outlined" elevation={0} sx={{ borderRadius: R, overflow: 'hidden' }}>
          {ml.filter(m => (!methodFilter || m.type_names.includes(methodFilter)) && `${m.name} ${m.full_name || ''} ${m.instrument_code || ''} ${m.instrument_name || ''}`.toLowerCase().includes(methodSearch.trim().toLowerCase())).map((m, index, filtered) => (
            <Box key={m.id} sx={{ display: 'grid', gridTemplateColumns: { xs: 'minmax(0,1fr) auto', md: 'minmax(260px,1.6fr) minmax(180px,1fr) minmax(180px,1fr) auto' }, alignItems: 'center', gap: 1.5, px: { xs: 1.25, md: 2 }, py: 1.1, borderBottom: index === filtered.length - 1 ? 'none' : '1px solid #edf0f2' }}>
              <Box sx={{ minWidth: 0 }}>
                <Box sx={{ display: 'flex', gap: 0.75, alignItems: 'center', flexWrap: 'wrap' }}>
                  ...
                  <Chip label={`仪器: ${m.instrument_code || '待配置'}`} size="small" color="info" sx={{ height: 22, borderRadius: R, fontSize: '0.68rem' }} />
                </Box>
              ...
              <Box sx={{ display: { xs: 'none', md: 'block' }, minWidth: 0 }}>
                ...
                <Box sx={{ display: 'flex', gap: 0.5, flexWrap: 'wrap' }}>{(m.type_names || []).map(t => <Chip key={t} label={t} size="small" variant="outlined" sx={{ height: 21, borderRadius: R, fontSize: '0.68rem' }} />)}</Box>
              </Box>
              ...
              <Box sx={{ display: 'flex', justifyContent: 'flex-end', gap: 0.25 }}>
                ...
                <IconButton size="small" color="error" onClick={() => openRecycleConfirm('methods', m.id, methodDisplayName(m), async (reason) => { const r = await deleteMethod(m.id, reason); if (r.code === 0) { sm('删除成功'); lm(); } else sm(r.message, true); setCo(false); })} aria-label={`删除${m.name}`}><DeleteIcon fontSize="small" /></IconButton>
              </Box>
            </Box>
          ))}
        </Paper>}
    ...
    {/* ── 回收站 ── */}
    {showSectionContent && tb === 'trash' && <Box>
      <Box sx={{ display: 'flex', alignItems: { xs: 'flex-start', sm: 'center' }, justifyContent: 'space-between', gap: 1, mb: 1.5 }}>
        <Box>
      ...
      {!trashLoading && trashItems.length > 0 && <>
        <TableContainer component={Paper} sx={{ ...tSx, display: { xs: 'none', sm: 'block' } }}>
          <Table size="small" sx={adaptiveTableSx}>
            ...
            <TableBody>{trashItems.map(entry => <TableRow key={entry.id} hover>
              ...
              <TableCell sx={adaptiveCellSx(trashRecordWidths.display_name)}><Typography variant="body2" fontWeight={700}>{entry.display_name}</Typography>{entry.business_no && entry.business_no !== entry.display_name && <Typography variant="caption" color="text.secondary">{entry.business_no}</Typography>}</TableCell>
              <TableCell sx={adaptiveCellSx(trashRecordWidths.module)}>{ML[entry.module] || entry.module}</TableCell>
        ...
        <Box sx={{ display: { xs: 'grid', sm: 'none' }, gap: 1 }}>
          {trashItems.map(entry => <Paper key={entry.id} variant="outlined" sx={{ p: 1.5, borderRadius: R }}>
            <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 1 }}><Box sx={{ minWidth: 0 }}><Typography fontWeight={800} sx={{ overflowWrap: 'anywhere' }}>{entry.display_name}</Typography><Typography variant="caption" color="text.secondary">{entry.entity_type} · {ML[entry.module] || entry.module}</Typography></Box><Chip size="small" label={TRASH_CATEGORIES[entry.category] || entry.category} /></Box>
            {entry.business_no && entry.business_no !== entry.display_name && <Typography variant="body2" sx={{ mt: 1, overflowWrap: 'anywhere' }}>业务编号：{entry.business_no}</Typography>}
    ...
    {/* ── 审计日志 ── */}
    {showSectionContent && tb === 'audit' && <Box>
      <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr 1fr', md: '150px 150px minmax(150px,1fr) minmax(190px,1.2fr) auto' }, gap: 1, mb: 2, alignItems: 'center' }}>
        <FormControl size="small"><InputLabel>业务模块</InputLabel><Select label="业务模块" value={auditFilters.module} onChange={e => setAuditFilters(v => ({ ...v, module: e.target.value }))}>
          <MenuItem value="">全部模块</MenuItem><MenuItem value="work">分析检测</MenuItem><MenuItem value="rd">研发送样</MenuItem><MenuItem value="sample_info">样品信息</MenuItem><MenuItem value="shared">主数据</MenuItem>
        </Select></FormControl>
      ...
      {al.length === 0 ? <Typography color="text.secondary" textAlign="center" sx={{ py: 4 }}>暂无审计日志</Typography> : <>
        <TableContainer component={Paper} className="table-responsive" sx={tSx}>
          <Table size="small" sx={adaptiveTableSx}>
            <TableHead><TableRow>
              ...
              <TableCell sx={{ ...adaptiveCellSx(auditLogWidths.user_name), fontWeight: 600 }}>操作人</TableCell>
            ...
            <TableBody>{al.map(l => (
              <TableRow key={l.id} hover>
                ...
                <TableCell sx={adaptiveCellSx(auditLogWidths.module)}>{ML[l.module] || l.module || '-'}</TableCell>
                <TableCell sx={adaptiveCellSx(auditLogWidths.action)}><Chip label={AL[l.action] || l.action} size="small" color={AC[l.action] || 'default'} variant="outlined" sx={{ borderRadius: R }} /></TableCell>
                ...
                <TableCell sx={adaptiveCellSx(auditLogWidths.user_name)}>{l.user_name}</TableCell>
        ...
        <DialogContent dividers>
          {auditDetail && <>
            <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr 1fr', md: 'repeat(4,1fr)' }, gap: 1.5, mb: 2 }}>
              <Box><Typography variant="caption" color="text.secondary">业务模块</Typography><Typography variant="body2">{ML[auditDetail.module] || auditDetail.module || '-'}</Typography></Box>
              <Box><Typography variant="caption" color="text.secondary">操作类型</Typography><Typography variant="body2">{AL[auditDetail.action] || auditDetail.action}</Typography></Box>
              <Box><Typography variant="caption" color="text.secondary">操作人</Typography><Typography variant="body2">{auditDetail.user_name}</Typography></Box>
            ...
            {traceLoading ? <CircularProgress size={22} /> : traceEvents.length === 0 ? <Typography variant="body2" color="text.secondary">该日志没有可用的记录时间线</Typography> : <Box sx={{ borderLeft: '2px solid #90caf9', ml: 1 }}>
              {traceEvents.map(event => <Box key={event.id} sx={{ position: 'relative', pl: 2, pb: 2, '&::before': { content: '""', position: 'absolute', left: -6, top: 5, width: 10, height: 10, borderRadius: '50%', bgcolor: '#1976d2' } }}>
                <Typography variant="body2" fontWeight={700}>{AL[event.event_type] || event.event_type}{event.from_status || event.to_status ? ` · ${event.from_status || '-'} → ${event.to_status || '-'}` : ''}</Typography>
    ...
    {/* ── 数据备份 ── */}
    {showSectionContent && tb === 'backup' && <Box>
      <Box sx={{ display: 'flex', gap: 2, mb: 3, flexWrap: 'wrap' }}>
        ...
        <Button variant="outlined" component="label" startIcon={<CloudUploadIcon />} sx={{ borderRadius: R, borderColor: '#f4511e', color: '#f4511e' }}>恢复备份<input type="file" accept=".db,.zip" hidden onChange={async (e) => { const f = e.target.files?.[0]; if (!f) return; if (!window.confirm(`确认恢复备份「${f.name}」？系统会先校验并生成恢复前备份，重启程序后正式生效。`)) { e.target.value = ''; return; } setBkR('校验并暂存中...'); try { const r = await restoreBackup(f); sm(r.message || '恢复已暂存'); await loadBk(); } catch (error: any) { sm(error?.message || '恢复失败', true); } finally { setBkR(''); e.target.value = ''; } }} /></Button>
        {bkR && <Chip label={bkR} color="warning" size="small" />}
      </Box>
      {bk?.pending_restore && <Alert severity="warning" sx={{ mb: 2 }}>存在已校验的待恢复备份，请重启程序完成数据恢复。</Alert>}
      ...
      <Paper elevation={0} sx={{ p: 2, mb: 3, borderRadius: R, border: '1px solid rgba(0,0,0,0.06)' }}>
        ...
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, flexWrap: 'wrap' }}>
          ...
          <Button size="small" variant="outlined" disabled={!bkSyncDir.trim()} onClick={async () => { try { const r = await testBackupSync(bkSyncDir); sm(r.message || '目录可用'); } catch (error: any) { sm(error?.message || '目录不可用', true); } }}>测试目录</Button>
          <Button size="small" variant="contained" onClick={async () => { try { const r = await updateBackupConfig({ enabled: bkAuto, interval_hours: bkInt, max_backup_count: maxBk, mode: bkMode, sync_dir: bkSyncDir || null }); sm(r.message || '设置已保存'); await loadBk(); } catch (error: any) { sm(error?.message || '保存失败', true); } }}>保存设置</Button>
        </Box>
      </Paper>
      {bk && bk.backup_files.length > 0 && <Paper elevation={0} sx={{ p: 2, borderRadius: R, border: '1px solid rgba(0,0,0,0.06)' }}>
        ...
        {bk.backup_files.map(f => <Box key={f.name} sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 1, py: 0.75, borderBottom: '1px solid rgba(0,0,0,0.04)' }}><Box sx={{ minWidth: 0 }}><Box sx={{ display: 'flex', alignItems: 'center', gap: 0.75 }}><Chip label={f.kind === 'full' || f.name.endsWith('.zip') ? '全量' : '数据库'} size="small" color={f.kind === 'full' || f.name.endsWith('.zip') ? 'primary' : 'default'} /><Typography variant="body2" sx={{ fontFamily: 'monospace', fontSize: '0.8rem', overflowWrap: 'anywhere' }}>{f.name}</Typography></Box><Typography variant="caption" color="text.secondary">{(f.size / 1024).toFixed(1)} KB · {f.time || ''}</Typography></Box><Box sx={{ display: 'flex', gap: 0.5, flexShrink: 0 }}><Button size="small" variant="outlined" onClick={async () => { if (!window.confirm(`确认恢复备份「${f.name}」？校验通过后将在重启程序时生效。`)) return; try { const r = await restoreBackupFile(f.name); sm(r.message || '恢复已暂存'); await loadBk(); } catch (error: any) { sm(error?.message || '恢复失败', true); } }}>恢复</Button><IconButton size="small" color="error" onClick={() => openRecycleConfirm('backup_files', 0, f.name, async (reason) => { try { const r = await deleteBackup(f.name, reason); sm(r.message || '已移入回收站'); await loadBk(); } catch (error: any) { sm(error?.message || '移入回收站失败', true); } finally { setCo(false); } })}><DeleteIcon fontSize="small" /></IconButton></Box></Box>)}
      </Paper>}
    </Box>}

    {/* ── 教程与帮助 ── */}
    {showSectionContent && tb === 'help' && <Box>
      {/* 上传区域 */}
      <Paper elevation={0} sx={{ p: 2, mb: 3, borderRadius: R, border: '1px solid rgba(0,0,0,0.06)' }}>
    ...
    {/* ── 样品信息登记管理 ── */}
    {showSectionContent && tb === 'sampleinfo' && <Box>
      ...
      {!siSubTab && (
        <Box sx={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: 2, mb: 3 }}>
          {[
            ...
            { key: 'rd_columns', label: '⑤ 研发送样列配置', desc: '配置研发送样表格列', icon: <ViewWeekIcon sx={{ fontSize: 36, color: '#e65100' }} />, color: '#e65100' },
          ].map(sc => (
      ...
      <Paper elevation={0} sx={{ p: 2, mb: 3, borderRadius: R, border: '1px solid rgba(0,0,0,0.08)' }}>
        ...
        {siTypeEdit !== null || siTypeNewOpen ? (
          <Box sx={{ display: 'flex', gap: 1.5, flexWrap: 'wrap', alignItems: 'center', mb: 2, p: 1.5, bgcolor: '#f5f9f5', borderRadius: R }}>
            ...
            <Button size="small" onClick={() => { setSiTypeEdit(null); setSiTypeNewOpen(false); setSiTypeForm({ type_key: '', label: '', description: '', color: '#2e7d32', sort_order: 0, is_active: 1 }); }} sx={{ borderRadius: R }}>取消</Button>
          </Box>
        ) : null}
      ...
      <Paper elevation={0} sx={{ p: 2, mb: 3, borderRadius: R, border: '1px solid rgba(0,0,0,0.08)' }}>
        ...
        <TableContainer component={Paper} sx={tSx}>
          <Table size="small" sx={adaptiveTableSx}>
            <TableHead><TableRow>
              <TableCell sx={{ ...adaptiveCellSx(siRecordWidths.seq_no), fontWeight: 600 }}>序号</TableCell>
              <TableCell sx={{ ...adaptiveCellSx(siRecordWidths.batch_no), fontWeight: 600 }}>批号</TableCell>
              <TableCell sx={{ ...adaptiveCellSx(siRecordWidths.user_name), fontWeight: 600 }}>送样人</TableCell>
              <TableCell sx={{ ...adaptiveCellSx(siRecordWidths.lab_name), fontWeight: 600 }}>实验室</TableCell>
              <TableCell sx={{ ...adaptiveCellSx(siRecordWidths.project_name), fontWeight: 600 }}>项目</TableCell>
              <TableCell sx={{ ...adaptiveCellSx(siRecordWidths.detection_type), fontWeight: 600 }}>检测类型</TableCell>
              <TableCell sx={{ ...adaptiveCellSx(siRecordWidths.status), fontWeight: 600 }}>状态</TableCell>
              <TableCell sx={{ ...adaptiveCellSx(siRecordWidths.detection_date), fontWeight: 600 }}>检测时间</TableCell>
              <TableCell sx={{ ...adaptiveCellSx(siRecordWidths.main_components), fontWeight: 600 }}>主要成分</TableCell>
            ...
            <TableBody>
              {siRecords.length === 0 ? (
                ...
                <TableRow key={r.id} hover>
                  {siEditId === r.id ? (
                    <>
                      <TableCell sx={adaptiveCellSx(siRecordWidths.seq_no)}>#{r.seq_no}</TableCell>
                      <TableCell sx={adaptiveCellSx(siRecordWidths.batch_no)}><TextField size="small" fullWidth value={siEditForm.batch_no || ''} onChange={e => setSiEditForm(p => ({ ...p, batch_no: e.target.value }))} /></TableCell>
                      <TableCell sx={adaptiveCellSx(siRecordWidths.user_name)}><TextField size="small" fullWidth value={siEditForm.user_name || ''} onChange={e => setSiEditForm(p => ({ ...p, user_name: e.target.value }))} /></TableCell>
                      <TableCell sx={adaptiveCellSx(siRecordWidths.lab_name)}><TextField size="small" fullWidth value={siEditForm.lab_name || ''} onChange={e => setSiEditForm(p => ({ ...p, lab_name: e.target.value }))} /></TableCell>
                      <TableCell sx={adaptiveCellSx(siRecordWidths.project_name)}><TextField size="small" fullWidth value={siEditForm.project_name || ''} onChange={e => setSiEditForm(p => ({ ...p, project_name: e.target.value }))} /></TableCell>
                      <TableCell sx={adaptiveCellSx(siRecordWidths.detection_type)}>{r.detection_type}</TableCell>
                      <TableCell sx={adaptiveCellSx(siRecordWidths.status)}>
                        <Chip label={r.status} size="small" color={r.status === '待取样' ? 'error' : r.status === '待检测' ? 'warning' : 'success'} />
                      </TableCell>
                      <TableCell sx={adaptiveCellSx(siRecordWidths.detection_date)}>{r.detection_date || '-'}</TableCell>
                      <TableCell sx={adaptiveCellSx(siRecordWidths.main_components)}><TextField size="small" fullWidth multiline minRows={1} value={siEditForm.main_components || ''} onChange={e => setSiEditForm(p => ({ ...p, main_components: e.target.value }))} /></TableCell>
                    ...
                    <>
                      <TableCell sx={adaptiveCellSx(siRecordWidths.seq_no)}>#{r.seq_no}</TableCell>
                      <TableCell sx={adaptiveCellSx(siRecordWidths.batch_no)}>{r.batch_no}</TableCell>
                      <TableCell sx={adaptiveCellSx(siRecordWidths.user_name)}>{r.user_name}</TableCell>
                      <TableCell sx={adaptiveCellSx(siRecordWidths.lab_name)}>{r.lab_name}</TableCell>
                      <TableCell sx={adaptiveCellSx(siRecordWidths.project_name)}>{r.project_name}</TableCell>
                      <TableCell sx={adaptiveCellSx(siRecordWidths.detection_type)}>{r.detection_type}</TableCell>
                      <TableCell sx={adaptiveCellSx(siRecordWidths.status)}><Chip label={r.status} size="small" color={r.status === '待取样' ? 'error' : r.status === '待检测' ? 'warning' : 'success'} /></TableCell>
                      <TableCell sx={adaptiveCellSx(siRecordWidths.detection_date)}>{r.detection_date || '-'}</TableCell>
                      <TableCell sx={adaptiveCellSx(siRecordWidths.main_components)}>{r.main_components}</TableCell>
                      <TableCell align="right" sx={adaptiveCellSx(siRecordWidths.actions)}>
                        <Box sx={{ display: 'flex', gap: 0.5, justifyContent: 'flex-end' }}>
          ...
          <TableContainer component={Paper} sx={tSx}>
            <Table size="small">
              <TableHead><TableRow>
                ...
                <TableCell sx={{ fontWeight: 600 }}>类型</TableCell>
                <TableCell sx={{ fontWeight: 600 }}>显示类型</TableCell>
              ...
              <TableBody>
                {siColumns.map((col, idx) => (
                  <TableRow key={col.id} hover>
                    ...
                    <TableCell>
                      <Box sx={{ display: 'flex', gap: 0.3, flexWrap: 'wrap', cursor: 'pointer' }}
                        onClick={(e) => {
                          setVisibleTypesColId(col.id);
                          setVisibleTypesSelected(col.visible_types ? [...col.visible_types] : []);
                          setVisibleTypesAnchorEl(e.currentTarget);
                        }}>
                        {col.visible_types && col.visible_types.length > 0 ? (
                          <>
                            {col.visible_types.slice(0, 3).map(tk => {
                              const st = sampleTypes.find(s => s.type_key === tk);
                              return <Chip key={tk} label={st?.label || tk} size="small" variant="outlined" sx={{ borderRadius: R, fontSize: '0.65rem', height: 20 }} />;
                            })}
                            {col.visible_types.length > 3 && (
                              <Chip label={`+${col.visible_types.length - 3}`} size="small" sx={{ borderRadius: R, fontSize: '0.65rem', height: 20, bgcolor: '#e0e0e0' }} />
                            )}
                          </>
                        ) : (
                          <Typography variant="caption" color="text.disabled">全部</Typography>
                        )}
                      </Box>
                    </TableCell>
      ...
      {/* 显示类型编辑弹窗 */}
      <Popover open={!!visibleTypesAnchorEl} anchorEl={visibleTypesAnchorEl}
        onClose={() => setVisibleTypesAnchorEl(null)}
        anchorOrigin={{ vertical: 'bottom', horizontal: 'left' }}
        slotProps={{ paper: { sx: { borderRadius: R, p: 1.5, minWidth: 200 } } }}>
        <Typography variant="subtitle2" sx={{ mb: 1, fontWeight: 600, fontSize: '0.8rem' }}>选择可见检测类型</Typography>
        <List dense sx={{ maxHeight: 240, overflow: 'auto', py: 0 }}>
          {sampleTypes.map(st => (
            <ListItem key={st.type_key} disableGutters sx={{ py: 0 }}>
              <ListItemIcon sx={{ minWidth: 36 }}>
                <Checkbox size="small" checked={visibleTypesSelected.includes(st.type_key)}
                  onChange={() => {
                    setVisibleTypesSelected(prev =>
                      prev.includes(st.type_key) ? prev.filter(t => t !== st.type_key) : [...prev, st.type_key]
                    );
                  }} />
              </ListItemIcon>
              <ListItemText primary={st.label} primaryTypographyProps={{ fontSize: '0.8rem' }} />
            </ListItem>
          ))}
        </List>
        <Box sx={{ display: 'flex', gap: 0.5, justifyContent: 'flex-end', mt: 1 }}>
          <Button size="small" onClick={() => setVisibleTypesAnchorEl(null)} sx={{ borderRadius: R, fontSize: '0.75rem' }}>取消</Button>
          <Button size="small" variant="contained" sx={{ borderRadius: R, fontSize: '0.75rem', bgcolor: '#2e7d32' }}
            onClick={async () => {
    ...
    {/* ── 主题设置 (v0.4.35) ── */}
    {showSectionContent && tb === 'theme' && <Box>
    ...
    {/* ── 首页配置 (v0.4.35) ── */}
    {showSectionContent && tb === 'home' && <Box>
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2 }}>
    ...
    {showSectionContent && tb === 'stats' && <Box>
      <Box sx={{ mb: 2 }}>
      ...
      <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', md: 'repeat(3, minmax(0, 1fr))' }, gap: 2 }}>
        {[
          ...
          return <Paper key={item.path} variant="outlined" sx={{ p: 2.25, borderRadius: R, borderTop: `3px solid ${item.color}`, display: 'flex', flexDirection: 'column', minHeight: 180 }}>
            <Box sx={{ width: 38, height: 38, display: 'grid', placeItems: 'center', color: item.color, bgcolor: `${item.color}14`, borderRadius: R, mb: 1.5 }}>{item.icon}</Box>
    ...
    {false && <Box>
      ...
      {STATS_PERMISSION_GROUPS.map(group => <Paper key={group.title} variant="outlined" sx={{ mb: 1.5, borderRadius: R, overflow: 'hidden' }}>
    ...
    {/* ── 用户管理 (v0.4.29) ── */}
    {showSectionContent && tb === 'users' && <Box>
      <Box sx={{ position: 'sticky', top: { xs: 112, md: 130 }, zIndex: 4, bgcolor: 'background.default', py: 1, display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2 }}>
        ...
        <Button variant="outlined" size="small" startIcon={<AddIcon />} onClick={() => { setImportDialogOpen(true); setImportFile(null); setImportUpdateExisting(false); setImportResult(null); }} sx={{ borderRadius: R, borderColor: '#2e7d32', color: '#2e7d32' }}>批量导入</Button>
      </Box>
      <Box sx={{ position: 'sticky', top: { xs: 164, md: 182 }, zIndex: 3, bgcolor: 'background.default', py: 1, display: 'flex', flexWrap: 'wrap', gap: 1, mb: 1.5 }}>
        <TextField size="small" placeholder="搜索用户名或用户 ID" value={userSearch} onChange={e => setUserSearch(e.target.value)} sx={{ minWidth: { xs: '100%', sm: 210 }, '& .MuiOutlinedInput-root': { borderRadius: R } }} />
        ...
        <Button size="small" onClick={() => { setUserSearch(''); setUserDivisionFilter(''); setUserAffiliationFilter(''); setUserGroupFilter(''); setUserRoleFilter(''); setUserStatusFilter('active'); }} sx={{ borderRadius: R }}>重置</Button>
      </Box>
        ...
        <Table size="small">
          ...
          <TableBody>
            ...
            {filteredUsers.length === 0 && (
              <TableRow><TableCell colSpan={10} align="center" sx={{ color: '#999', py: 3 }}>{users.length === 0 ? '暂无用户' : '没有符合当前筛选条件的用户'}</TableCell></TableRow>
            )}
          </TableBody>
        </Table>
      </TableContainer>
    ...
    <Dialog open={projectEditOpen} onClose={() => { setProjectEditOpen(false); setProjectEditItem(null); }} maxWidth="md" fullWidth PaperProps={{ sx: { borderRadius: R } }}>
      ...
      <DialogContent>
        {projectEditItem && <>
                    ...
                    {typeMethods.map(m => (
                      <FormControlLabel
                        ...
                        control={<Checkbox
                          ...
                          size="small"
                        />}
                        label={<Typography variant="caption">{methodDisplayName(m)}</Typography>}
                        sx={{ mr: 2, mb: 0.5 }}
                      />
                    ))}
                  </FormGroup>
          ...
          {/* v0.4.64: 高项自定义填写 */}
          <TextField label="关联高项（自定义填写）" fullWidth size="small" placeholder="例如：A 类/重点/高项" value={projectEditItem.high_item || ''} onChange={e => setProjectEditItem({ ...projectEditItem, high_item: e.target.value })} sx={{ mt: 2, '& .MuiOutlinedInput-root': { borderRadius: R } }} />
    ...
    <ResponsiveEditDrawer open={methodEditOpen} title={methodEditItem?.id ? '编辑方法' : '新建方法'} onClose={() => { setMethodEditOpen(false); setMethodEditItem(null); }} onSave={() => { if (methodEditItem) handleSaveMethod(methodEditItem); }}>
      <Box>
        {methodEditItem && <>
          <TextField label="方法名称" required fullWidth value={methodEditItem.name} onChange={e => setMethodEditItem({ ...methodEditItem, name: e.target.value })} sx={{ mt: 2, mb: 2, '& .MuiOutlinedInput-root': { borderRadius: R } }} helperText="只填写方法名称；同名方法通过对应仪器区分" />
          <FormControl required fullWidth sx={{ mb: 2, '& .MuiOutlinedInput-root': { borderRadius: R } }}>
          ...
          <Table size="small" stickyHeader>
            ...
            <TableBody>{instruments.map(i => <TableRow key={i.id} hover>
              ...
              <TableCell align="right">
                ...
                <Tooltip title="删除"><IconButton size="small" color="error" onClick={() => openRecycleConfirm('instruments', i.id, i.code, async (reason) => { const r = await deleteInstrument(i.id, reason); if (r.code === 0) { sm('删除成功'); await li(); } else sm(r.message, true); setCo(false); })}><DeleteIcon fontSize="small" /></IconButton></Tooltip>
              </TableCell>
            </TableRow>)}</TableBody>
          </Table>
        </TableContainer>
    ...
    <Dialog open={divEditOpen} onClose={() => setDivEditOpen(false)} maxWidth="sm" fullWidth PaperProps={{ sx: { borderRadius: R } }}>
      ...
      <DialogContent>
        <TextField label="部门名称" fullWidth value={divForm.name} onChange={e => setDivForm({ ...divForm, name: e.target.value })} sx={{ mt: 2, mb: 2, '& .MuiOutlinedInput-root': { borderRadius: R } }} helperText="如: 研究院、化工一部、化工二部、动保一部、动保二部" />
            ...
            <TableBody>
              {mts.map(t => <TableRow key={t.id} hover>
                ...
                <TableCell align="right">
                  ...
                  <IconButton size="small" color="error" onClick={() => openRecycleConfirm('method_types', t.id, t.name, async (reason) => { const r = await deleteMethodType(t.id, reason); if (r.code === 0) { sm('删除成功'); lmt(); } else sm(r.message, true); setCo(false); })}><DeleteIcon fontSize="small" /></IconButton>
                </TableCell>
              </TableRow>)}
            </TableBody>
          </Table>
    ...
    {/* v0.4.104: Excel 用户导入对话框 */}
    <Dialog open={importDialogOpen} onClose={() => setImportDialogOpen(false)} maxWidth="sm" fullWidth PaperProps={{ sx: { borderRadius: R } }}>
      <DialogTitle sx={{ fontWeight: 700 }}>批量导入用户</DialogTitle>
      <DialogContent>
        <Box sx={{ mb: 2 }}>
          <Typography variant="body2" color="text.secondary" sx={{ mb: 1 }}>① 下载系统生成的 Excel 模板，角色候选来自当前程序</Typography>
          <Button variant="outlined" size="small" startIcon={<DownloadIcon />} onClick={async () => {
            try { await downloadUserImportTemplate(); sm('用户导入模板已下载'); }
            catch (error) { sm(error instanceof Error ? error.message : '模板下载失败', true); }
          }} sx={{ borderRadius: R }}>下载用户导入模板（.xlsx）</Button>
        </Box>
        <Box sx={{ mb: 2 }}>
          <Typography variant="body2" color="text.secondary" sx={{ mb: 1 }}>② 填写“用户导入”工作表后上传。每个用户可填写多个角色；角色不存在时只跳过该角色。</Typography>
          <Button variant="outlined" component="label" sx={{ borderRadius: R }}>
            选择文件
            <input type="file" hidden accept=".xlsx" onChange={e => { setImportFile(e.target.files?.[0] || null); setImportResult(null); e.target.value = ''; }} />
          </Button>
          {importFile && <Chip label={importFile.name} size="small" color="primary" variant="outlined" sx={{ ml: 1, borderRadius: R }} onDelete={() => setImportFile(null)} />}
        </Box>
        <FormControlLabel control={<Checkbox checked={importUpdateExisting} onChange={e => setImportUpdateExisting(e.target.checked)} />} label="更新已有用户的部门、实验室与角色（不修改密码）" />
        {importResult && (
          <Paper elevation={0} sx={{ p: 2, bgcolor: '#f5f9f5', borderRadius: R, border: '1px solid rgba(0,0,0,0.06)' }}>
            <Typography variant="body2" sx={{ color: '#2e7d32' }}>✓ 成功导入 {importResult.created} 条</Typography>
            {(importResult.updated || 0) > 0 && <Typography variant="body2" sx={{ color: '#1976d2' }}>✓ 更新已有用户 {importResult.updated} 条</Typography>}
            {importResult.skipped > 0 && <Typography variant="body2" sx={{ color: '#f57c00' }}>⚠ 跳过 {importResult.skipped} 条</Typography>}
            <Typography variant="body2" color="text.secondary">已映射角色 {importResult.roles_mapped || 0} 个，未匹配/无权限角色 {importResult.roles_ignored || 0} 个，无角色用户 {importResult.users_without_roles || 0} 个</Typography>
            {(importResult.warnings || []).map((e: string, i: number) => <Typography key={`warning-${i}`} variant="caption" display="block" color="warning.main">{e}</Typography>)}
            {(importResult.errors || []).map((e: string, i: number) => <Typography key={i} variant="caption" display="block" color="error">{e}</Typography>)}
          </Paper>
        )}
      </DialogContent>
      <DialogActions>
        <Button onClick={() => { setImportDialogOpen(false); setImportFile(null); setImportUpdateExisting(false); setImportResult(null); }} sx={{ borderRadius: R }}>关闭</Button>
        <Button variant="contained" disabled={!importFile} onClick={async () => {
          if (!importFile) return;
          try { const r = await importUsers(importFile, importUpdateExisting); if (r.code === 0) { setImportResult(r.data); userList().then(r2 => { if (r2.code === 0 && r2.data) setUsers(r2.data); }).catch(() => {}); } else { sm(r.message, true); } }
          catch { sm('导入失败', true); }
        }} sx={{ borderRadius: R, bgcolor: '#2e7d32' }}>开始导入</Button>
      </DialogActions>
    </Dialog>
    {showSectionContent && tb === 'layouts' && <PageLayoutAdmin />}
    {showSectionContent && tb === 'forms' && <ManageFormConfig />}
    {showSectionContent && tb === 'exports' && <ManageExportConfig />}
    {showSectionContent && tb === 'sessions' && <SessionsPanel onMessage={sm} />}
    {showSectionContent && tb === 'log-maintenance' && <LogMaintenancePanel onMessage={sm} />}
    {showSectionContent && tb === 'master-import' && <MasterImportPanel onImported={async () => { await Promise.all([ldiv(), lg(), lp(), lm(), lmt()]); }} />}
      </Box>
  </Box>
