  ...
  const ld2 = useCallback(async () => {
    ...
    try {
      if (ac === "week" || ac === "month") {
        ...
        const r = await getRdPreviewSheet2({ start: si, end: ei });
        if (r.code === 0) setS2d(r.data || []);
      } else if (ac === "sheet3") {
        const r = await getRdPreviewSheet3({ start: si, end: ei });
        if (r.code === 0) setS3d(r.data || []);
      } else if (ac === "sheet4") {
        const r = await getRdPreviewSheet4({ start: si, end: ei });
        if (r.code === 0) setS4d(r.data || []);
      } else if (ac === "sheet5") {
        const r = await getRdPreviewSheet5({ start: si, end: ei });
        if (r.code === 0) setS5d(r.data || []);
      } else if (ac === "sheet6") {
        const r = await getRdPreviewSheet6({ start: si, end: ei });
        if (r.code === 0) setS6d(r.data || []);
      } else if (ac === "sheet7") {
        const r = await getRdPreviewSheet7({ start: si, end: ei });
        if (r.code === 0) setS7d(r.data || []);
      } else if (ac === "sheet8") {
        const r = await getRdPreviewSheet8({ start: si, end: ei });
        if (r.code === 0) setS8d(r.data || []);
      } else if (ac === "sheet9") {
        const r = await getRdPreviewSheet9({ start: si, end: ei });
        if (r.code === 0) setS9d(r.data || []);
      } else if (ac === "sheet10") {
        const r = await getRdPreviewSheet10({ start: si, end: ei });
        if (r.code === 0) setS10d(r.data || []);
      } else if (ac === "division" || ac === "sheet11") {
        const r = await getRdPreviewSheet11({ start: si, end: ei });
    ...
    (async () => {
      try {
        const [rt, rp, ri, ru, rd] = await Promise.all([
          getRdStatsByType({ start: si, end: ei }),
          getRdStatsByProject({ start: si, end: ei }),
          getRdStatsByInstrument({ start: si, end: ei }),
          getRdStatsByUser({ start: si, end: ei }),
          getRdStatsByDivision({ start: si, end: ei }),
        ]);
