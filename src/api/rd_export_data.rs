...
pub fn query_sheet1_data(
    ...
    sql.push_str(" GROUP BY pg.id, pg.name, p.id, m.id, wr.instrument_code_snapshot, wr.instrument_type_snapshot ORDER BY lab_name, p.name, m.name");
...
mod tests {
    ...
    fn rd_amount_sheets_use_method_multiplier_and_type_sheet_does_not() {
        ...
        assert_eq!(
            query_sheet11_data(&conn, "2026-07-17", "2026-07-17", None, None).unwrap()[0]
                .multiplier,
            1.0
        );
        assert_eq!(
            query_sheet3_data(&conn, "2026-07-17", "2026-07-17", Some(10), None)
                .unwrap()
                .len(),
            1
        );
        assert!(
            query_sheet3_data(&conn, "2026-07-17", "2026-07-17", Some(11), None)
                .unwrap()
                .is_empty()
        );
        assert_eq!(
            query_sheet3_data(&conn, "2026-07-17", "2026-07-17", None, Some(1))
                .unwrap()
                .len(),
            1
        );
        assert!(
            query_sheet3_data(&conn, "2026-07-17", "2026-07-17", None, Some(2))
                .unwrap()
                .is_empty()
        );
    }
