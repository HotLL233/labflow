...
fn export_scope(pool: &DbPool, headers: &HeaderMap) -> Result<(Option<i64>, Option<i64>)> {
...
async fn preview_sheet1(
    ...
    let conn = pool.get()?;
    let (user_id, group_id) = export_scope(&pool, &headers)?;
    let data =
        rd_export_data::query_sheet1_data(&conn, &start, &end, q.group_id, user_id, group_id)?;
...
async fn preview_sheet2(
    ...
    let conn = pool.get()?;
    let (user_id, group_id) = export_scope(&pool, &headers)?;
    let data = rd_export_data::query_sheet2_data(&conn, &start, &end, user_id, group_id)?;
    Ok(Json(ApiResponse::ok(data)))
}
...
async fn preview_sheet3(
    ...
    let conn = pool.get()?;
    let (user_id, group_id) = export_scope(&pool, &headers)?;
    let data = rd_export_data::query_sheet3_data(&conn, &start, &end, user_id, group_id)?;
    Ok(Json(ApiResponse::ok(data)))
}
...
async fn preview_sheet4(
    ...
    let conn = pool.get()?;
    let (user_id, group_id) = export_scope(&pool, &headers)?;
    let data = rd_export_data::query_sheet4_data(&conn, &start, &end, user_id, group_id)?;
    Ok(Json(ApiResponse::ok(data)))
}
...
async fn preview_sheet5(
    ...
    let conn = pool.get()?;
    let (user_id, group_id) = export_scope(&pool, &headers)?;
    let data = rd_export_data::query_sheet5_data(&conn, &start, &end, user_id, group_id)?;
    Ok(Json(ApiResponse::ok(data)))
}
...
async fn preview_sheet6(
    ...
    let conn = pool.get()?;
    let (user_id, group_id) = export_scope(&pool, &headers)?;
    let data = rd_export_data::query_sheet6_data(&conn, &start, &end, user_id, group_id)?;
    Ok(Json(ApiResponse::ok(data)))
}
...
async fn preview_sheet7(
    ...
    let conn = pool.get()?;
    let (user_id, group_id) = export_scope(&pool, &headers)?;
    let data = rd_export_data::query_sheet7_data(&conn, &start, &end, user_id, group_id)?;
    Ok(Json(ApiResponse::ok(data)))
}
...
async fn preview_sheet8(
    ...
    let conn = pool.get()?;
    let (user_id, group_id) = export_scope(&pool, &headers)?;
    let data = rd_export_data::query_sheet8_data(&conn, &start, &end, user_id, group_id)?;
    Ok(Json(ApiResponse::ok(data)))
}
...
async fn preview_sheet9(
    ...
    let conn = pool.get()?;
    let (user_id, group_id) = export_scope(&pool, &headers)?;
    let data = rd_export_data::query_sheet9_data(&conn, &start, &end, user_id, group_id)?;
    Ok(Json(ApiResponse::ok(data)))
}
...
async fn preview_sheet10(
    ...
    let conn = pool.get()?;
    let (user_id, group_id) = export_scope(&pool, &headers)?;
    let data = rd_export_data::query_sheet10_data(&conn, &start, &end, user_id, group_id)?;
    Ok(Json(ApiResponse::ok(data)))
}
...
async fn preview_sheet11(
    ...
    let conn = pool.get()?;
    let (user_id, group_id) = export_scope(&pool, &headers)?;
    let data = rd_export_data::query_sheet11_data(&conn, &start, &end, user_id, group_id)?;
    Ok(Json(ApiResponse::ok(data)))
}
