...
fn require_editor(pool: &DbPool, headers: &HeaderMap) -> Result<authz_service::AuthContext> {
    ...
    if !ctx.is_rd_leader() {
        return Err(AppError::Forbidden("\u{4ec5}\u{7814}\u{53d1}\u{9001}\u{6837}\u{7ec4}\u{957f}\u{53ef}\u{65b0}\u{589e}\u{3001}\u{7f16}\u{8f91}\u{6216}\u{64a4}\u{56de}\u{672c}\u{5b9e}\u{9a8c}\u{5ba4}\u{4eba}\u{5458}\u{53d8}\u{52a8}\u{53cd}\u{9988}".into()));
    }
    Ok(ctx)
}
fn assert_lab(ctx: &authz_service::AuthContext, lab_id: i64) -> Result<()> {
    if ctx.user.group_id == Some(lab_id) {
        Ok(())
    } else {
