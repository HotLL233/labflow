...
fn list_all_on_conn(conn: &postgres_compat::Connection) -> Result<Vec<SampleInfoColumn>> {
    let mut stmt = conn.prepare(
        "SELECT c.id, c.field_key, c.label, c.data_type, c.is_predefined, c.is_required, c.is_active, \
         ...
         COALESCE((SELECT string_agg(v.type_key, ',' ORDER BY v.type_key) FROM sample_info_column_visibility v WHERE v.column_id=c.id), '') AS visible_types \
...
pub fn create(pool: &DbPool, data: &ColumnCreate, operator: &str) -> Result<SampleInfoColumn> {
    ...
    tx.execute(
        "INSERT OR IGNORE INTO sample_info_column_visibility (column_id, type_key, is_visible)
         SELECT ?1, type_key, 1 FROM sample_info_types WHERE is_active = 1",
        params![id],
    )?;
...
pub fn set_visibility(
    ...
    for tk in type_keys {
        tx.execute(
            "INSERT INTO sample_info_column_visibility (column_id, type_key, is_visible) VALUES (?1, ?2, 1)",
            params![column_id, tk],
        )?;
    }
    let updated = list_all_on_conn(&tx)?
