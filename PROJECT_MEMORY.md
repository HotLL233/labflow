...
> 最后更新：2026-09-23
>
> 当前正式基线：`v2.3.22`（记录表列宽改版；v2.3.22 修正表头折行折算与压缩下限）
>
> 当前源码目录：`source/LabFlow-PostgreSQL-v2.3.22/`
...
内容：修复 v2.3.20 引入、v2.3.21 未修好的记录表列宽异常（详见 `source/LabFlow-PostgreSQL-v2.3.22/更新说明.md`）。

- 现象：表头逐字竖排（「序号」占两行、「检测类型」占四行），各列被压到 20~30px；超长表头列反而比短列宽。
- 根因（两处独立缺陷，v2.3.21 只修了「容器宽度恒为 0」）：
  1. 表头按**单行**宽度参与分配；后台把 label 配成「注意事项（理化检测样品标注大致含量）」（约 280px）后，被上限截到 240px 并独占空间。
  2. 压缩**没有下限保护**：整体等比压缩把自然宽已等于下限的「序号」压到 28px 以下，2 个字只能逐字竖排。
- 修复（**长期规则，后续不得回退**）：
  - 表头宽度参与计算时**必须按折行折算**（>120px 时取 `min(表头宽, 200)/2`），禁止按单行原始宽度分配。
  - 压缩必须按「可压缩量 = 目标宽 − 下限」比例扣减，**任何列不得低于自身下限**；全部到下限后保持下限并接受横向滚动，不得继续压缩到内容不可读。
  - 管理员 `custom` 模式的列不参与压缩。
  - 表头加 `title` 属性，超长表头可悬停查看全文。
